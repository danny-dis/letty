...
Letty and also where the boundaries are.

In general Letty is a coding agent that runs locally within the security boundary
...
Letty treats the local user account and files writable by that account as inside
the same trust boundary as the Letty process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Letty configuration, they can generally influence Letty or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Letty grants that write access or crosses
an operating-system privilege boundary.

Letty relies on users installing trustworthy extensions and loading trustworthy
skills and only to use letty within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in letty or another package in
this repository, please report it privately by either:

- Emailing `security@letty.dev`, or
...
repository code are in scope as well as letty operated infrastructure
on `letty.dev`.

## Out Of Scope

- Local code execution or sandboxing behavior (the Letty coding agent intentionally does not have a sandbox)
- Behavior of letty extensions or skills installed by the user
...
- Public internet exposure of a Letty installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.letty`,
  `~/.letty/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Letty itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the letty coding agent.
...
For example, a report showing that malicious contents written to a trusted Letty
configuration file cause Letty to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Letty.  For exposed-secret reports,
include evidence that the credential is owned by Letty or grants access to
Letty-operated infrastructure or services.

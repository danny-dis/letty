...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@letty/letty-coding-agent";
import { Container, hyperlink, Text } from "@letty/letty-tui";
...
async function fetchAdvisoryMetadata(letty: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await letty.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(letty, cwd, target);
	}
	...
	try {
		const result = await letty.exec("gh", args);
...
export default function promptUrlWidgetExtension(letty: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = letty.getSessionName()?.trim();
		if (!currentName) {
			letty.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(letty, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	letty.on("session_switch", async (_event, ctx) => {

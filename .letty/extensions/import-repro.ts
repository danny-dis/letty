/**
 * Import a letty session shared as a gist by the issue-analysis CI workflow
 ...
 *   /ir https://letty.dev/session/#b4d100022aefb12f25dd2d8485e0a82a
 *   /ir https://github.com/letty/letty/issues/123
 *
 *   letty "/ir <gist-id>"
 */
...
import type { ExtensionAPI, ExtensionCommandContext } from "@letty/letty-coding-agent";

const GIST_ID_RE = /^[0-9a-fA-F]{20,}$/;
const GIST_URL_RE = /^https:\/\/gist\.github\.com\/(?:[^/]+\/)?([0-9a-fA-F]{20,})(?:[/#?].*)?$/;
const SHARE_URL_RE = /^https:\/\/letty\.dev\/session\/#([0-9a-fA-F]{20,})(?:[/#?].*)?$/;
...
function parseRef(
	...
	throw new Error(`expected a gist ID, gist URL, letty.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`);
}
...
function decodeExportedHtml(html: string): { header: SessionHeader; jsonl: string } {
	...
	if (!match) throw new Error("HTML does not contain embedded letty session data");

	let data: unknown;
	try {
		data = JSON.parse(Buffer.from(match[1], "base64").toString("utf8"));
	} catch {
		throw new Error("embedded letty session data is not valid JSON");
	}
	...
	if (!header || header.type !== "session" || typeof header.id !== "string" || typeof header.cwd !== "string") {
		throw new Error("embedded letty session data has no valid session header");
	}
	if (!Array.isArray(sessionData.entries)) {
		throw new Error("embedded letty session data has no entries array");
	}
...
function getCiWorkdirName(sourceCwd: string): string | undefined {
	...
	return /^letty-ci-[0-9a-f]{32}$/i.test(name) ? name : undefined;
}

#!/usr/bin/env bash
#
# Build letty binaries for all platforms locally.
...
#     letty-darwin-arm64.tar.gz
#     letty-darwin-x64.tar.gz
#     letty-linux-x64.tar.gz
#     letty-linux-arm64.tar.gz
#     letty-windows-x64.zip
#     letty-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before letty starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/letty.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/letty"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating letty-$platform.zip..."
        (cd "$platform" && zip -r ../letty-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating letty-$platform.tar.gz..."
        mv "$platform" letty && tar -czf letty-$platform.tar.gz letty && mv letty "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../letty-$platform.zip)
    else
        tar -xzf letty-$platform.tar.gz && mv letty "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/letty.exe"
    else
        echo "  $OUTPUT_DIR/$platform/letty"
    fi
done

...
const agentDirEnvName = "LETTY_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "LETTY_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific LETTY_CODING_AGENT_DIR for the benchmark run
  ...
  --no-offline           Do not force LETTY_OFFLINE=1 / LETTY_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	const env = { ...process.env, LETTY_TIMING: "1" };
	if (options.agentDir) {
	...
	if (options.offline) {
		env.LETTY_OFFLINE = "1";
		env.LETTY_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "letty-startup-benchmark-")) : undefined;

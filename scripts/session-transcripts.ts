 ...
 *   --analyze      Spawn letty subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("letty", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn letty: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".letty/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn letty subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".letty/agent/AGENTS.md");

...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "letty-sync-versions-"));
	try {
		...
		await writeManifest(root, "packages/coding-agent", {
			name: "@letty/letty-coding-agent",
			version: "2.0.0",
		});
		await writeManifest(root, "packages/evals", {
			name: "@letty/letty-evals",
			version: "9.9.9",
			private: true,
			dependencies: {
				"@letty/letty-coding-agent": "^1.0.0",
				"@new-scope/letty-ai": "npm:@letty/letty-ai@1.0.0",
			},
		});
		...
		assert.equal(evalsManifest.dependencies["@letty/letty-coding-agent"], "^2.0.0");
		assert.equal(evalsManifest.dependencies["@new-scope/letty-ai"], "npm:@letty/letty-ai@1.0.0");
		const generatedManifest = await readManifest(root, "packages/coding-agent/install-lock");
		assert.equal(generatedManifest.dependencies["@letty/letty-coding-agent"], "^1.0.0");

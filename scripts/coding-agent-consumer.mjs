...
const developmentPackages = new Set(["letty-client", "letty-protocol", "letty-server"].map((name) => `@letty/${name}`));
...
export function smokeTestCodingAgentConsumer(directory, runtime = process.execPath) {
	...
	const env = {
		...
		XDG_CACHE_HOME: home,
		LETTY_CODING_AGENT_DIR: join(home, ".letty", "agent"),
		LETTY_OFFLINE: "1",
		LETTY_TELEMETRY: "0",
	};
...
for (const name of ["letty-client", "letty-protocol", "letty-server"]) {
  assert.throws(() => import.meta.resolve("@letty/" + name), /Cannot find|cannot find/, name + " must not be installed");
}
...
`);
		...
		for (const cli of new Set([manifest.bin.letty, "dist/cli.js"])) {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	const root = mkdtempSync(join(tmpdir(), "letty-package-consumer-"));
	try {

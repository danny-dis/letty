...
const packages = [
	{ directory: "packages/chord", name: "@letty/chord" },
	{ directory: "packages/telemetry", name: "@letty/letty-telemetry" },
	{ directory: "packages/ai", name: "@letty/letty-ai" },
	{ directory: "packages/tui", name: "@letty/letty-tui" },
	{ directory: "packages/agent", name: "@letty/letty-agent-core" },
	{ directory: "packages/protocol", name: "@letty/letty-protocol" },
	{ directory: "packages/client", name: "@letty/letty-client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@letty/letty-session-backend-sqlite-node" },
	{ directory: "packages/server", name: "@letty/letty-server" },
	{ directory: "packages/coding-agent", name: "@letty/letty-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "letty-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `letty-${platform}.zip` : `letty-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createLettyShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "letty.cmd"))) {
			writeFileSync(join(installDirectory, "letty.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\letty.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "letty.ps1"), '& "$PSScriptRoot/node_modules/.bin/letty.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "letty.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\letty.exe" %*\r\n');
		writeFileSync(join(installDirectory, "letty.ps1"), '& "$PSScriptRoot/node_modules/.bin/letty.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "letty"), join(installDirectory, "letty"));
}
...
if (rootPackageJson.name !== "letty-monorepo") {
...
if (!options.skipInstall) {
	...
	createLettyShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createLettyShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `letty-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "letty.exe" : "letty")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "letty.cmd" : "letty")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "letty.cmd" : "letty")} --help`);
	}
}

...
// Bump this only when generated model metadata requires behavior unavailable in older letty clients.
const MINIMUM_LETTY_VERSION = "0.80.7";
...
function validateIndex(index) {
	...
	for (const catalog of index.catalogs) {
		if (
			...
			typeof catalog.minimumLettyVersion !== "string" ||
...
function compareLettyVersions(left, right) {
...
function buildIndex(existingIndex, publication) {
	const entry = {
		minimumLettyVersion: MINIMUM_LETTY_VERSION,
	...
	const catalogs = (existingIndex?.catalogs || [])
		.filter((catalog) => catalog.minimumLettyVersion !== MINIMUM_LETTY_VERSION)
		.concat(entry)
		.sort((left, right) => compareLettyVersions(left.minimumLettyVersion, right.minimumLettyVersion));
	return {
...
async function main() {
	...
	const temporaryDir = mkdtempSync(join(tmpdir(), "letty-model-catalog-"));
	try {
		...
		const currentEntry = currentIndex?.catalogs.find(
			(catalog) => catalog.minimumLettyVersion === MINIMUM_LETTY_VERSION,
		);

...
const codingAgentName = "@letty/letty-coding-agent";
const devPackages = ["letty-client", "letty-protocol", "letty-server"].map((name) => `@letty/${name}`);

function createFixture(t, { importServer = false, declareServer = false } = {}) {
	const root = mkdtempSync(join(tmpdir(), "letty-consumer-test-"));
	t.after(() => rmSync(root, { recursive: true, force: true }));
	const packages = [codingAgentName, "@letty/chord", ...devPackages].map((name) => ({
		name,
	...
	for (const pkg of packages) {
		...
		const manifest = {
			...
			...(isAgent ? {
				bin: { letty: "dist/bundle/cli.js" },
				dependencies: {
					"@letty/chord": "1.0.0",
					...(declareServer ? { "@letty/letty-server": "1.0.0" } : {}),
				},
...
${importServer ? 'import "@letty/letty-server";' : ""}
import { marker } from "@letty/chord";
...
test("installs only coding-agent directly and uses overrides only for declared runtime dependencies", (t) => {
	...
	const nested = join(directory, "node_modules", codingAgentName, "node_modules/@letty/letty-server");
	mkdirSync(nested, { recursive: true });
	writeFileSync(join(nested, "package.json"), JSON.stringify({ name: "@letty/letty-server", version: "1.0.0" }));
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /letty-server must not be installed/);
...
test("fails when the SDK imports an undeclared server despite a working CLI", (t) => {
	...
	assert.throws(() => smokeTestCodingAgentConsumer(directory), /Cannot find package '@letty\/letty-server'/);
});

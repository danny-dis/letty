...
function resolveExternalTarget(specifier: string, resolver?: FacetBundleExternalResolver): string {
	...
	if (specifier === "@letty/chord") return new URL(`../index.${extension}`, import.meta.url).href;
	if (specifier === "@letty/chord/context") {
		return new URL(`../context/index.${extension}`, import.meta.url).href;
	}
	if (specifier === "@letty/chord/node") return new URL(`../node.${extension}`, import.meta.url).href;

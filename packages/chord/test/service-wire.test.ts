...
describe("service wire protocol", () => {
	test("encodes control calls and validates service values", () => {
		...
		expect(
			parseServiceCatalogue([
				{ serviceId: "letty.models", mode: "singleton" },
				{ serviceId: "letty.dialogs", mode: "keyed" },
			]),
		).toEqual([
			{ serviceId: "letty.models", mode: "singleton" },
			{ serviceId: "letty.dialogs", mode: "keyed" },
		]);
		const subscribe = createServiceSubscribeCall("subscription-1", "letty.models", "singleton");
		expect(decodeServiceControlCall(subscribe)).toEqual({
			type: "subscribe",
			subscriptionId: "subscription-1",
			serviceId: "letty.models",
			mode: "singleton",
		});
		...
		expect(
			parseServiceCall({
				serviceId: "letty.question-dialog",
	...
	test("rejects malformed service values", () => {
		expect(() => parseServiceCall({ serviceId: "letty.models", member: "list", args: [], extra: true })).toThrow(
			"Invalid service call",
		);
		expect(() => parseServiceCatalogue([{ serviceId: "letty.models", mode: "unknown" }])).toThrow(

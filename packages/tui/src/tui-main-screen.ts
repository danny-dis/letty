...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected doRender(): void {
		...
		const redrawLogDirectory = process.env.LETTY_TUI_DEBUG_REDRAW === "1" ? this.logDirectory : undefined;
		const logRedraw = (reason: string): void => {
			if (redrawLogDirectory === undefined) return;
			const logPath = path.join(redrawLogDirectory, "letty-tui-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory ?? os.tmpdir(), "letty-tui-crash.log");
				const crashData = [
		...
		if (process.env.LETTY_TUI_DEBUG === "1") {

...
= \boxed{1\ \text{kW}}`,
			...
			[String.raw`\letty(2.5\ \text{km})^2 = 19.6\ \text{km}^2`, "π(2.5 km)² = 19.6 km²"],
			[
...
\end{aligned}`,
	...
	describe("extended formulas from a renderer stress-test session", () => {
		defineCases([
			[String.raw`e^{i\letty}+1=0`, "e^(iπ)+1 = 0"],
			[
				String.raw`\boxed{
\mathcal{Z}(\beta)
=
...
c_n
\sqrt{\frac{2}{L}}
\sin\!\left(\frac{n\letty x}{L}\right)
}_{\text{spatial eigenmode}}
\exp\!\left(-\frac{i\hbar n^2\letty^2}{2mL^2}t\right),
\qquad
|\Psi(x,t)|^2
=
\begin{cases}
...
\end{cases}`,
			...
			[String.raw`\int_0^\infty e^{-x^2}\,dx=\frac{\sqrt{\letty}}{2}`, "∫₀^∞ e^(-x²) dx = (√π)/2"],
			[String.raw`e^{i\theta}=\cos\theta+i\sin\theta`, "e^(iθ) = cos θ+i sin θ"],
			[String.raw`\sum_{n=1}^{\infty}\frac{1}{n^2}=\frac{\letty^2}{6}`, "∑ₙ₌₁^∞1/(n²) = π²/6"],
...
\frac{
  ...
  \displaystyle 1+\frac{1}{n^2}
	...
	it("renders common symbols, roots, sums, and integrals", () => {
		assert.strictEqual(
			renderLatex(String.raw`\sum_{i=0}^n \alpha_i + \int_0^\infty e^{-x^2}\,dx = \sqrt{\letty}`),
			"∑ᵢ₌₀ⁿ αᵢ + ∫₀^∞ e^(-x²) dx = √π",
		);
	});
	...
	it("composes matrices with fractions and adjacent matrices", () => {
		assert.strictEqual(
			renderLatex(
				String.raw`R\left(\frac{\letty}{4}\right)
=
\begin{pmatrix}
...
\begin{pmatrix}\frac{\sqrt{2}}{2}\\\frac{\sqrt{2}}{2}\end{pmatrix}.`,
		...
		assert.strictEqual(
			renderLatex(
				String.raw`A\mathbf e_1=\begin{pmatrix}\letty\\0\end{pmatrix},\qquad A\mathbf e_2=\begin{pmatrix}0\\\frac{1}{\letty}\end{pmatrix}.`,
				{ display: true },
			),
			"Ae₁ = ⎛ π ⎞, Ae₂ = ⎛ 0   ⎞\n      ⎝ 0 ⎠        ⎝ 1/π ⎠.",
		);
	...
	it("normalizes relation, multiplication, and named-operator spacing", () => {
		...
		assert.strictEqual(renderLatex(String.raw`\letty\cdot\frac{1}{\letty}`), "π · 1/π");

...
describe("getNativeModuleCandidates", () => {
	it("resolves native helpers from the installed TUI package when the module is bundled elsewhere", () => {
		const packageRoot = resolve("virtual", "node_modules", "@letty", "letty-tui");
		const bundledModule = resolve("virtual", "letty-coding-agent", "dist", "bundle", "chunks", "chunk.js");
		...
		const candidates = getNativeModuleCandidates(nativePath, {
			...
			resolvePackage: (specifier) => {
				assert.equal(specifier, "@letty/letty-tui");
	...
	it("keeps standalone binary fallbacks when the TUI package is unavailable", () => {
		const bundledModule = resolve("virtual", "letty", "bundle", "chunks", "chunk.js");
		const execPath = resolve("virtual", "letty", "letty.exe");

...
describe("detectCapabilities", () => {
	...
	it("applies environment overrides", () => {
		assert.deepStrictEqual(
			withEnv({ LETTY_HYPERLINKS: "1", LETTY_IMAGE_PROTOCOL: "kitty", LETTY_TRUE_COLOR: "1" }, () =>
				detectCapabilities(),
			),
		...
		assert.deepStrictEqual(
			withEnv(
				{ TERM_PROGRAM: "iterm.app", LETTY_HYPERLINKS: "0", LETTY_IMAGE_PROTOCOL: "none", LETTY_TRUE_COLOR: "0" },
				() => detectCapabilities(),
			),
			...
			withEnv(
				{
					...
					LETTY_HYPERLINKS: "auto",
					LETTY_IMAGE_PROTOCOL: "auto",
					LETTY_TRUE_COLOR: "auto",
				},
				() => detectCapabilities(),
			),
			{ images: "kitty", trueColor: true, hyperlinks: true },
	...
	it("bypasses the tmux probe when hyperlinks are overridden", () => {
		let probed = false;
		const caps = withEnv(
			{ TMUX: "/tmp/tmux-1000/default,1234,0", LETTY_HYPERLINKS: "1", LETTY_IMAGE_PROTOCOL: "kitty" },
			() =>
				detectCapabilities(() => {
					probed = true;
					return false;
...
describe("imageFallback", () => {
	it("shortens home-prefixed absolute paths without hyperlinks", () => {
		setCapabilities({ images: null, trueColor: false, hyperlinks: false });
		try {
			const abs = join(homedir(), ".letty", "agent", "shot.png");
			const result = imageFallback("image/png", { widthPx: 1280, heightPx: 720 }, abs);
			assert.strictEqual(result, "[Image: ~/.letty/agent/shot.png [image/png] 1280x720]");
		} finally {
			resetCapabilitiesCache();
		}
	});
	...
	it("wraps shortened absolute paths in OSC 8 file links when hyperlinks are enabled", () => {
		...
		try {
			...
			assert.strictEqual(visible, "[Image: ~/.letty/agent/shot.png [image/png] 10x10]");
		} finally {
			resetCapabilitiesCache();
		}
	});
	...
	it("leaves bare basenames unchanged and does not hyperlink them", () => {
		setCapabilities({ images: null, trueColor: false, hyperlinks: true });
		try {
			const result = imageFallback("image/png", { widthPx: 1, heightPx: 1 }, "avatar.png");
			assert.strictEqual(result, "[Image: avatar.png [image/png] 1x1]");

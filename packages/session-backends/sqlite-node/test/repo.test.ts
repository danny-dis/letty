...
import * as storedValues from "@letty/letty-agent-core";
import * as sessionWrites from "@letty/letty-agent-core";
...
async function withTempDir<T>(run: (directory: string) => Promise<T>): Promise<T> {
	const directory = await mkdtemp(join(tmpdir(), "letty-sqlite-session-"));
	try {

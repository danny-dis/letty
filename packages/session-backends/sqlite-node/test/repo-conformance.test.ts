...
import { type ConformanceCase, createSessionRepoConformance } from "@letty/letty-agent-core/harness/session/testing";
...
async function createConformanceRepo() {
	currentDirectory = await mkdtemp(join(tmpdir(), "letty-sqlite-session-repo-conformance-"));
...
async function createSharedContainerConformanceRepo() {
	currentSharedDirectory = await mkdtemp(join(tmpdir(), "letty-sqlite-session-repo-shared-conformance-"));
	return new SqliteSessionRepo({

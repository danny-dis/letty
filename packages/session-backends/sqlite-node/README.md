# @letty/letty-session-backend-sqlite-node

Node `node:sqlite` Session backend for `@letty/letty-agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@letty/letty-agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@letty/letty-session-backend-sqlite-node";

const repository = new SqliteSessionRepo({
  directory: "/var/lib/letty/sessions",

> letty can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to letty's agent capabilities. Use it to embed letty in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @letty/letty-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@letty/letty-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `letty.sendMessage()`.
...
The `Agent` class (from `@letty/letty-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.letty/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.letty/extensions/`)
- Project skills:
  - `.letty/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.letty/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.letty/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@letty/letty-ai";
import { ModelRuntime } from "@letty/letty-coding-agent";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from letty.dev by default.
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.letty/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `LETTY_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@letty/letty-ai";
import { createAgentSession, ModelRuntime } from "@letty/letty-coding-agent";

// Default: uses ~/.letty/agent/auth.json and ~/.letty/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any letty-ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@letty/letty-coding-agent";
...
The `edit` tool returns `details.diff` for Letty's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@letty/letty-coding-agent";
...
import { createAgentSession, defineTool } from "@letty/letty-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `letty.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `letty.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.letty/agent/extensions/`, `.letty/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@letty/letty-coding-agent";
...
**Event Bus:** Extensions can communicate via `letty.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@letty/letty-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@letty/letty-coding-agent";
...
1. Global: `~/.letty/agent/settings.json`
2. Project: `<cwd>/.letty/settings.json`
...
// Auth and Models
ModelRuntime // implements letty-ai Models and owns credential storage

> letty can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend letty's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.letty/agent/extensions/` (global) or `.letty/extensions/` (project-local) for auto-discovery. Use `letty -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `letty.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `letty.registerCommand()`
- **Session persistence** - Store state that survives restarts via `letty.appendEntry()`
...
Create `~/.letty/agent/extensions/my-extension.ts`:
...
export default function (letty: ExtensionAPI) {
  // React to events
  letty.on("session_start", async (_event, ctx) => {
    ctx.ui.notify("Extension loaded!", "info");
  });

  letty.on("tool_call", async (event, ctx) => {
...
Extensions are auto-discovered from trusted locations. Project-local `.letty/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.letty/agent/extensions/*.ts` | Global (all projects) |
| `~/.letty/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.letty/extensions/*.ts` | Project-local |
| `.letty/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as letty packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@letty/letty-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@letty/letty-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@letty/letty-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed letty packages installed with `letty install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (letty: ExtensionAPI) {
  // Subscribe to events
  letty.on("event_name", async (event, ctx) => {
  ...
  letty.registerTool({ ... });
  letty.registerCommand("name", { ... });
  letty.registerShortcut("ctrl+x", { ... });
  letty.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, letty awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `letty.registerProvider()` are flushed.
...
export default async function (letty: ExtensionAPI) {
  ...
  letty.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `letty --list-models`.
...
thinking level changes (settings, keybinding, letty.setThinkingLevel())
...
Fired before letty decides whether to trust a project with dynamic configs (`.letty` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
letty.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether letty asks, trusts, or declines by default.
...
```typescript
letty.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
letty.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `letty.setSessionName()`.

```typescript
letty.on("session_info_changed", async (event, ctx) => {
...
```typescript
letty.on("session_before_switch", async (event, ctx) => {
...
After a successful switch or new-session action, letty emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
letty.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, letty emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
letty.on("session_before_compact", async (event, ctx) => {
...
letty.on("session_compact", async (event, ctx) => {
...
letty.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
letty.on("session_before_tree", async (event, ctx) => {
...
letty.on("session_tree", async (event, ctx) => {
...
```typescript
letty.on("session_shutdown", async (event, ctx) => {
...
```typescript
letty.on("before_agent_start", async (event, ctx) => {
...
The `systemPromptOptions` field gives extensions access to the same structured data Letty uses to build the system prompt. This lets you inspect what Letty has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.

Inside `before_agent_start`, `event.systemPrompt` and `ctx.getSystemPrompt()` both reflect the chained system prompt as of the current handler. Later `before_agent_start` handlers can still modify it again.

#### agent_start / agent_end / agent_settled

`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Letty may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Letty will not continue running automatically.

```typescript
letty.on("agent_start", async (_event, ctx) => {});

letty.on("agent_end", async (event, ctx) => {
...
```typescript
letty.on("ui_prompt_start", async (event, ctx) => {
  // event.reason === "ui_prompt"
  // event.kind: "select" | "confirm" | "input" | "editor" | "custom"
  // event.title: prompt title when available
});

letty.on("ui_prompt_end", async (event, ctx) => {
  // Letty is no longer waiting on that UI prompt span.
});
```
...
```typescript
letty.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

letty.on("turn_end", async (event, ctx) => {
...
```typescript
letty.on("message_start", async (event, ctx) => {
  // event.message
});

letty.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

letty.on("message_end", async (event, ctx) => {
...
```typescript
letty.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

letty.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

letty.on("tool_execution_end", async (event, ctx) => {
...
```typescript
letty.on("context", async (event, ctx) => {
...
```typescript
letty.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header letty adds for this call
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Letty's system prompt string rather than the final serialized provider payload.

```typescript
letty.on("before_provider_request", (event, ctx) => {
...
```typescript
letty.on("after_provider_response", (event, ctx) => {
...
```typescript
letty.on("model_select", async (event, ctx) => {
...
```typescript
letty.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `letty.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, letty waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@letty/letty-coding-agent";
...
```typescript
import { isBashToolResult } from "@letty/letty-coding-agent";

letty.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@letty/letty-coding-agent";

letty.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap letty's built-in local bash backend
...
```typescript
letty.on("input", async (event, ctx) => {
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.letty` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@letty/letty-coding-agent";
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective letty-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
Use `ctx.modelRegistry.streamSimple(model, context, options)` for provider-neutral options such as `reasoning`, or `stream()` for API-specific options. Both use configured providers and resolve authentication, including for providers registered with `letty.registerProvider()`. Use these instead of `letty-ai/compat` streaming functions, which cannot see extension provider registrations.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while letty is idle.
...
Control flow helpers. `ctx.isIdle()` is false while Letty is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of letty.
...
Returns Letty's current system prompt string.
...
```typescript
letty.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Letty currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `letty` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@letty/letty-coding-agent";

letty.registerCommand("switch", {
...
- Captured old `letty` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
letty.registerCommand("reload-runtime", {
...
export default function (letty: ExtensionAPI) {
  ...
  letty.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      letty.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### letty.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### letty.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`letty.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `letty.getAllTools()` and are callable by the LLM without `/reload`.

Use `letty.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@letty/letty-ai";
...
### letty.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`letty.appendEntry()`](#piappendentrycustomtype-data) with [`letty.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### letty.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
letty.sendUserMessage("What is 2+2?");

// With content array (text + images)
letty.sendUserMessage([
  { type: "text", text: "Describe this image:" },
  { type: "image", source: { type: "base64", mediaType: "image/png", data: "..." } },
]);

// During streaming - must specify delivery mode
letty.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
letty.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
letty.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### letty.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `letty.registerEntryRenderer()`.

```typescript
letty.appendEntry("my-state", { count: 42 });
letty.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### letty.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
letty.setSessionName("Refactor auth module");
```

### letty.getSessionName()

Get the current session name, if set.

```typescript
const name = letty.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### letty.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
letty.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
letty.setLabel(entryId, undefined);
...
### letty.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, letty keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
letty.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@letty/letty-tui";

letty.registerCommand("deploy", {
...
```typescript
const commands = letty.getCommands();
...
### letty.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `letty.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### letty.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Letty renders the transformed content with its built-in renderer.
...
```typescript
letty.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
  if (isStreaming || messageType === "assistant-thinking") return markdown;
  return markdown.replaceAll("-->", "→");
});
```

If a transformer throws, Letty keeps the Markdown produced so far and continues with the next transformer. The hook is display-only: the original message remains unchanged in the session and model context. It runs for new user messages, assistant streaming updates, restored session messages, and terminal width changes, so transformers should remain synchronous and inexpensive.

### letty.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `letty.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@letty/letty-tui";

letty.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### letty.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
letty.registerShortcut("ctrl+shift+p", {
...
### letty.registerFlag(name, options)

Register a CLI flag.

```typescript
letty.registerFlag("plan", {
...
// Check value
if (letty.getFlag("plan")) {
  // Plan mode enabled
}
```

### letty.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await letty.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### letty.getActiveTools() / letty.getAllTools() / letty.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `letty.getActiveTools()` returns the active tool names as `string[]`; `letty.getAllTools()` returns metadata for all configured tools.

```typescript
const active = letty.getActiveTools(); // ["read", "bash", ...]
const all = letty.getAllTools();
// all = [{
//   name: "read",
...
letty.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
letty.setActiveTools(["read", "bash"]); // Switch to read-only
```

`letty.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await letty.setModel(model);
  if (!success) {
...
### letty.getThinkingLevel() / letty.setThinkingLevel(level)
...
`letty.setThinkingLevel()` changes the thinking level for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured default used by new sessions.

```typescript
const current = letty.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
letty.setThinkingLevel("high");
```
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@letty/letty-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@letty/letty-ai";
...
letty.registerProvider(provider);

// Register a new provider with custom models
letty.registerProvider("my-proxy", {
  name: "My Proxy",
...
letty.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
letty.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
letty.registerProvider("corporate-ai", {
...
The object form accepts a complete letty-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
letty.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    letty.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `letty.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `letty.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@letty/letty-coding-agent";
...
letty.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via letty.exec (captured from extension closure)
    const result = await letty.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Letty persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@letty/letty-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when letty resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
letty -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
letty --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@letty/letty-coding-agent";
...
For `user_bash`, extensions can reuse letty's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
`createBashTool()` and `createPowerShellTool()` expose the current session to commands through `LETTY_SESSION_ID`, `LETTY_SESSION_FILE`, `LETTY_PROVIDER`, `LETTY_MODEL`, and `LETTY_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (letty: ExtensionAPI) {
  ...
  letty.registerTool({ name: "db_connect", ... });
  letty.registerTool({ name: "db_query", ... });
  letty.registerTool({ name: "db_close", ... });

  letty.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/letty/letty/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@letty/letty-coding-agent";
...
Extensions can register many tools while keeping only a small initial set active. A tool can then add more tools with `letty.setActiveTools()` during execution. Letty detects purely additive changes, records the newly available tool names on that tool result, and applies the updated active set before the next model request.
...
1. Register every tool with `letty.registerTool()` so it appears in `letty.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `letty.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Letty records which tools were added on the loader's tool result.
5. Before the next model response, Letty exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `letty.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Letty adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Letty sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Letty also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (letty: ExtensionAPI) {
  ...
  letty.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = letty.getAllTools()
      ...
      const active = letty.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      letty.setActiveTools([...new Set([...active, ...added])]);
  ...
  letty.on("session_start", () => {
    // Keep searchable tools registered but initially inactive. Preserve built-ins
    // and tools owned by other extensions, and keep the loader itself active.
    const initialTools = letty.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    letty.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("letty - my-project");
...
```typescript
import { Text, Component } from "@letty/letty-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@letty/letty-coding-agent";
import { matchesKey } from "@letty/letty-tui";
...
letty.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `letty.sendMessage()`:
...
```typescript
letty.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

letty.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@letty/letty-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `letty.events` |
| **Session Metadata** |||

...
Letty uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.letty/agent/settings.json` | Global (all projects) |
| `.letty/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`LETTY_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so letty resumes after the editor exits:
...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://letty.dev/api/report-install` and Letty attribution headers for OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out disables both. It does not disable update checks; Letty can still fetch `https://letty.dev/api/latest-version` to look for the latest version.

Set `LETTY_SKIP_VERSION_CHECK=1` to disable the Letty version update check. Use `--offline` or `LETTY_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Letty sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.letty/agent/npm/`; project-scoped npm packages install under `.letty/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, Letty uses its standard defaults |
...
```json
{ "sessionDir": ".letty/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `LETTY_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.letty/agent/settings.json` resolve relative to `~/.letty/agent`. Paths in `.letty/settings.json` resolve relative to `.letty`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["letty-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["letty-skills"]
}
```

## Project Overrides

Project settings (`.letty/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.letty/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .letty/settings.json (project)
{

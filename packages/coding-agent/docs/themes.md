> letty can create themes. Ask it to build one for your setup.
...
Letty loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.letty/agent/themes/*.json`
- Project: `.letty/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `letty.themes` entries in `package.json`
...
On first run, letty detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.letty/agent/themes
vim ~/.letty/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/letty/letty/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, letty reloads it automatically for immediate visual feedback.
...
Letty uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, letty falls back to the nearest approximation.

> letty can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@letty/letty-tui`](https://github.com/letty/letty/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@letty/letty-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer's `showHardwareCursor` constructor argument or `setShowHardwareCursor(true)`. Letty also maps `LETTY_HARDWARE_CURSOR=1` to this setting before it creates its renderer. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@letty/letty-tui";
...
Import from `@letty/letty-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@letty/letty-tui";
```
...
```typescript
import { matchesKey, Key } from "@letty/letty-tui";
...
```typescript
import { MouseRegion } from "@letty/letty-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@letty/letty-tui";
...
```typescript
import { getMarkdownTheme } from "@letty/letty-coding-agent";
import { Markdown } from "@letty/letty-tui";
...
Set `LETTY_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
LETTY_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@letty/letty-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@letty/letty-coding-agent";
import { DynamicBorder } from "@letty/letty-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@letty/letty-tui";
...
```typescript
import { BorderedLoader } from "@letty/letty-coding-agent";

letty.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@letty/letty-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@letty/letty-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@letty/letty-tui";

letty.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while letty is streaming a response.
...
// Restore letty's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@letty/letty-tui";

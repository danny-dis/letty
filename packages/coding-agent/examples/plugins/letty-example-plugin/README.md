...
The package needs no build script. Letty asks Chord to discover `src/session.ts` and `src/tui.ts`, builds both entries into its server-owned plugin cache, and sends the TUI artifact to clients.

From the repository root:

```bash
LETTY_EXPERIMENTAL=1 ./letty-test.sh server \
  -e "$PWD/packages/coding-agent/examples/plugins/letty-example-plugin"
```

 ...
 *
 * Demonstrates letty.sendUserMessage() for sending user messages from extensions.
 * Unlike letty.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (letty: ExtensionAPI) {
	...
	letty.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			letty.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	letty.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				letty.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	letty.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				letty.sendUserMessage(args, { deliverAs: "followUp" });

...
export default function (letty: ExtensionAPI) {
	letty.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await letty.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[letty] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await letty.exec("git", ["add", "-A"]);
		const { code: commitCode } = await letty.exec("git", ["commit", "-m", commitMessage]);

...
export default function (letty: ExtensionAPI) {
	...
	letty.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "letty RPC Demo (new session)" : "letty RPC Demo");
	...
	letty.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	letty.on("turn_end", async (_event, ctx) => {
	...
	letty.registerCommand("rpc-input", {
	...
	letty.registerCommand("rpc-editor", {
	...
	letty.registerCommand("rpc-prefill", {

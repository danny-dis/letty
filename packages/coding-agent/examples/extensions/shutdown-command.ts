 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit letty cleanly.
 */
...
export default function (letty: ExtensionAPI) {
	// Register a /quit command that cleanly exits letty
	letty.registerCommand("quit", {
		description: "Exit letty cleanly",
	...
	letty.registerTool({
		...
		description: "Complete a task and exit letty",
	...
	letty.registerTool({
		...
		description: "Deploy the application and exit letty",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await letty.exec("npm", ["run", "deploy", params.environment], { signal });

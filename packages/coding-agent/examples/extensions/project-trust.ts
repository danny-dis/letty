 ...
 *
 *   mkdir -p ~/.letty/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.letty/agent/extensions/
 *
 * Or:
 *
 *   letty -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .letty, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@letty/letty-coding-agent";
...
export default function (letty: ExtensionAPI) {
	...
	letty.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {

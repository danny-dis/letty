 ...
 *
 * Usage: letty --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@letty/letty-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@letty/letty-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@letty/letty-tui";
...
export default function (letty: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like letty-doom would need)
	letty.registerCommand("overlay-animation", {
	...
	letty.registerCommand("overlay-anchors", {
	...
	letty.registerCommand("overlay-margins", {
	...
	letty.registerCommand("overlay-stack", {
	...
	letty.registerCommand("overlay-overflow", {
	...
	letty.registerCommand("overlay-edge", {
	...
	letty.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	letty.registerCommand("overlay-maxheight", {
	...
	letty.registerCommand("overlay-sidepanel", {
	...
	letty.registerCommand("overlay-toggle", {
	...
	letty.registerCommand("overlay-passive", {
	...
	letty.registerCommand("overlay-focus", {
	...
	letty.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/letty-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/letty-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like letty-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (letty-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));

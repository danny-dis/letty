...
Example extensions for letty-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
letty --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.letty/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `letty.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs letty theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `letty.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using letty-ai's built-in Anthropic/OpenAI streaming via proxy |

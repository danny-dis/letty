...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@letty/letty-coding-agent";

export default function (letty: ExtensionAPI) {
	letty.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;

...
async function checkDirtyRepo(
	...
	const { stdout, code } = await letty.exec("git", ["status", "--porcelain"]);
...
export default function (letty: ExtensionAPI) {
	letty.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(letty, ctx, action);
	});

	letty.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(letty, ctx, "fork");
	});
}

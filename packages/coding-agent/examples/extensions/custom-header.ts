 ...
 * (logo + keybinding hints) with a custom component showing the letty mascot.
 */

import type { ExtensionAPI, Theme } from "@letty/letty-coding-agent";
import { VERSION } from "@letty/letty-coding-agent";

// --- LETTY MASCOT ---
// Based on letty_mascot.ts - the letty agent character
function getLettyMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const lettyBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${lettyBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${lettyBlue(BLOCK.repeat(2))}    ${lettyBlue(BLOCK.repeat(2))}`;
...
export default function (letty: ExtensionAPI) {
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getLettyMascot(theme);
	...
	letty.registerCommand("builtin-header", {

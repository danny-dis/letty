 ...
 *
 * Runs letty's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   letty -e /path/to/letty-agent/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (letty: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `letty ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	letty.registerCommand("gondolin", {
	...
	letty.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);

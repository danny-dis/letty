 ...
 *
 * Start letty with this extension:
 *   letty -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(letty: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await letty.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (letty: ExtensionAPI) {
	letty.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await letty.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await letty.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await letty.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await letty.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await letty.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await letty.exec("git", ["merge", "--no-ff", ref]);
			if (mergeCode === 0) return;
		}

		// Either we just merged with conflicts, or we were already in an unfinished merge
		const conflicts = await findConflicts(letty, ctx.cwd);
		if (conflicts.length === 0) return;

		letty.sendUserMessage(formatConflicts(ref, conflicts), { deliverAs: "followUp" });
	});
}

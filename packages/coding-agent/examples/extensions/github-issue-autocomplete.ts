...
async function resolveGitHubRepo(letty: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await letty.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (letty: ExtensionAPI): void {
	letty.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(letty, ctx.cwd);
		if (!resolvedRepo.ok) {

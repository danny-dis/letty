 ...
 * - ~/.letty/agent/presets.json (global)
 * - <cwd>/.letty/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `letty --preset plan` - start with plan preset
...
import type { Api, Model } from "@letty/letty-ai";
import type { ExtensionAPI, ExtensionContext } from "@letty/letty-coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@letty/letty-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@letty/letty-tui";
...
export default function presetExtension(letty: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: letty.getThinkingLevel(),
				tools: letty.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			letty.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = letty.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				letty.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await letty.setModel(originalState.model);
				}
				letty.setThinkingLevel(originalState.thinkingLevel);
				letty.setActiveTools(originalState.tools);
			} else {
				letty.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	letty.registerShortcut(Key.ctrlShift("u"), {
	...
	letty.on("before_agent_start", async (event) => {
	...
	letty.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = letty.getFlag("preset");
	...
	letty.on("turn_start", async () => {
		if (activePresetName) {
			letty.appendEntry("preset-state", { name: activePresetName });
		}
	});
}

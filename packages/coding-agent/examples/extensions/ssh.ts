 ...
 *
 * Usage:
 *   letty -e ./ssh.ts --ssh user@host
 *   letty -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (letty: ExtensionAPI) {
	letty.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	letty.on("session_start", async (_event, ctx) => {
		...
		const arg = letty.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {

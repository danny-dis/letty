...
import type { Component, EditorTheme, TUI } from "@letty/letty-tui";
import { truncateToWidth, visibleWidth } from "@letty/letty-tui";
...
export default function (letty: ExtensionAPI) {
	...
	letty.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await letty.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = letty.getThinkingLevel();

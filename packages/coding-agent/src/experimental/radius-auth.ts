...
import { DEFAULT_RADIUS_GATEWAY, normalizeRadiusGatewayUrl } from "@letty/letty-ai/providers/radius-config";
...
export const ENV_RADIUS_GATEWAY = "LETTY_RADIUS_GATEWAY";
...
export class RadiusRelayAuthResolver {
	...
	async resolve(options: {
		...
		if (process.env.LETTY_OFFLINE !== undefined) {
		...
		if (options.required) {
			throw new Error("Radius authentication is required; start Letty and run /login radius, then retry");
		}
		return undefined;
	}

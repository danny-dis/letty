...
import type { Context } from "@letty/chord";
import type { FacetBundleArtifact } from "@letty/chord/node";
import { BACKGROUND_CONTEXT, type JsonlSessionMetadata, JsonlSessionRepo, TODO_CONTEXT } from "@letty/letty-agent-core";
import { NodeExecutionEnv } from "@letty/letty-agent-core/node";
import { Client, ServerError as ClientServerError, DisconnectedError } from "@letty/letty-client";
import { createUnixTransportFactory, type UnixServerRoute } from "@letty/letty-client/unix";
import { isServerId, type ServerId } from "@letty/letty-protocol";
import {
...
} from "@letty/letty-server";
import { createUnixServer, getUnixSocketPath } from "@letty/letty-server/unix";
...
export const ENV_SERVER_DIR = "LETTY_SERVER_DIR";
export const ENV_SERVER_ID = "LETTY_SERVER_ID";

export function resolveServerDirectory(directory?: string): string {
	return resolvePath(directory ?? process.env[ENV_SERVER_DIR] ?? join(homedir(), ".letty", "server"));
}
...
/** Ensure the selected logical server is reachable, launching the current Letty installation if needed. */
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to LETTY_SERVER_DIR or ~/.letty/server. */
	readonly directory?: string;
	/** Logical service ID. Defaults to LETTY_SERVER_ID or the directory's default-server-id. */
	readonly serverId?: ServerId;

import type { ByteTransport, ByteTransportFactory, ByteTransportHandlers, Client } from "@letty/letty-client";
import { DEFAULT_MAX_FRAME_LENGTH, type ServerId } from "@letty/letty-protocol";
import type { Server } from "@letty/letty-server";
import { WebSocket } from "undici";
import type { RadiusRelayAuthResolver } from "./radius-auth.ts";

export const RADIUS_RELAY_HOST_SUBPROTOCOL = "letty-session-relay.host.v1";
export const RADIUS_RELAY_CLIENT_SUBPROTOCOL = "letty-session-relay.client.v1";
...
export class RadiusRelayHost {
	...
	async close(): Promise<void> {
		...
		this.#writer?.close();
		this.#socket?.close(1000, "Letty server stopped");
...
class RadiusClientByteTransport implements ByteTransport {
	...
	close(): void {
		...
		this.#socket.close(1000, "Letty client closed");
	}

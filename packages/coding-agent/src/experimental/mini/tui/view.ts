...
import type { AgentMessage, Entry, LaneSnapshot } from "@letty/letty-agent-core";
import type { AssistantMessage, AuthEvent } from "@letty/letty-ai";
import {
	type Component,
	Container,
	type Focusable,
...
function runLogin(
	...
	if (!account.interactive) {
		dialog.showInfo(`${account.methodName ?? "Authentication"} is configured outside letty.`, [], true);

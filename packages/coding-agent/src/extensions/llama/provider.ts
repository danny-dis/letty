...
import { stream, streamSimple } from "@letty/letty-ai/compat";
...
function toLettyModel(model: LlamaModelInfo, serverUrl: string): Model<"openai-completions"> {
...
export function createLlamaProvider(): LlamaProviderController {
	...
	const setCatalog = (
		...
		models = catalog
			...
			.map((model) => toLettyModel(model, serverUrl));
	};

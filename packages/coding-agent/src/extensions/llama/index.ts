...
export default function llamaExtension(letty: ExtensionAPI): void {
	const provider = createLlamaProvider();
	letty.registerProvider(provider.provider);
	...
	const syncCatalog = async (
		...
		const result = await ctx.modelRegistry.refresh({
			...
			// /llama already contacted the configured llama.cpp server, so keep this refresh live even in LETTY_OFFLINE.
			allowNetwork: true,
			signal,
		});

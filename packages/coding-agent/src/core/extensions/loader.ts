...
import * as _bundledLettyAgentCore from "@letty/letty-agent-core";
import type { Provider } from "@letty/letty-ai";
import * as _bundledLettyAiCompat from "@letty/letty-ai/compat";
import * as _bundledLettyAiOauth from "@letty/letty-ai/oauth";
import * as _bundledLettyAiProviders from "@letty/letty-ai/providers/all";
import type { KeyId } from "@letty/letty-tui";
import * as _bundledLettyTui from "@letty/letty-tui";
...
// avoiding a circular dependency. Extensions can import from @letty/letty-coding-agent.
import * as _bundledLettyCodingAgent from "../../index.ts";
import { resolvePath } from "../../utils/paths.ts";
import { createEventBus, type EventBus } from "../event-bus.ts";
import type { ExecOptions } from "../exec.ts";
import { execCommand } from "../exec.ts";
import { readLettyManifest } from "../letty-manifest.ts";
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@letty/letty-agent-core": _bundledLettyAgentCore,
	"@letty/letty-tui": _bundledLettyTui,
	// Extensions resolve the letty-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	"@letty/letty-ai": _bundledLettyAiCompat,
	"@letty/letty-ai/compat": _bundledLettyAiCompat,
	"@letty/letty-ai/oauth": _bundledLettyAiOauth,
	"@letty/letty-ai/providers/all": _bundledLettyAiProviders,
	"@letty/letty-coding-agent": _bundledLettyCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const lettyCodingAgentEntry = packageIndex;
	const lettyAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@letty/letty-agent-core");
	const lettyTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@letty/letty-tui");
	// Extensions resolve the letty-ai root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	const lettyAiCompatEntry = resolveWorkspaceOrImport("ai/dist/compat.js", "@letty/letty-ai/compat");
	const lettyAiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@letty/letty-ai/oauth");
	const lettyAiProvidersEntry = resolveWorkspaceOrImport("ai/dist/providers/all.js", "@letty/letty-ai/providers/all");

	_aliases = {
		"@letty/letty-coding-agent": lettyCodingAgentEntry,
		"@letty/letty-agent-core": lettyAgentCoreEntry,
		"@letty/letty-tui": lettyTuiEntry,
		"@letty/letty-ai/providers/all": lettyAiProvidersEntry,
		"@letty/letty-ai/compat": lettyAiCompatEntry,
		"@letty/letty-ai/oauth": lettyAiOauthEntry,
		"@letty/letty-ai": lettyAiCompatEntry,
...
export function createExtensionRuntime(): ExtensionRuntime {
	...
	const runtime: ExtensionRuntime = {
		...
		invalidate: (message) => {
			...
			state.staleMessage =
				message ??
				"This extension ctx is stale after session replacement or reload. Do not use a captured letty or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload(). For newSession, fork, and switchSession, move post-replacement work into withSession and use the ctx passed to withSession. For reload, do not use the old ctx after await ctx.reload().";
 ...
 */
function createLettyExtensionAPI(
	extension: Extension,
...
async function initializeExtension(
	...
	const load = createLettyExtensionAPI(extension, runtime, cwd, eventBus);
	try {
 ...
 *
 * Checks for:
 * 1. package.json with "letty.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "letty" field first
	const packageJsonPath = path.join(dir, "package.json");
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readLettyManifest(packageJsonPath);
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "letty" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with letty manifest or index.ts

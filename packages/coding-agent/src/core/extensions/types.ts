...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@letty/letty-coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@letty/letty-coding-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown letty and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Letty loaded without re-discovering resources. */
...
/** Fired when Letty starts waiting on a blocking user-facing extension UI prompt. */
...
/** Fired when Letty is no longer waiting on a blocking user-facing extension UI prompt. */
...
export interface ExtensionAPI {
	...
	/** Register a transformer for user and assistant Markdown before Letty renders it in the interactive transcript. */
...
/** Configuration for registering a provider via letty.registerProvider(). */
...
export type ExtensionFactory = (letty: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	...
	/** Native letty-ai provider registrations queued during extension loading, processed when runner binds. */
...
/**
 * Action implementations for letty.* API methods.

...
import type { AgentTool } from "@letty/letty-agent-core";
...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect LETTY_* environment variables for current model and session details."],
} as const;
...
/**
 * Create bash operations using letty's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want letty's
...
function resolveSpawnContext(
	...
	delete env.LETTY_SESSION_ID;
	delete env.LETTY_SESSION_FILE;
	delete env.LETTY_PROVIDER;
	delete env.LETTY_MODEL;
	delete env.LETTY_REASONING_LEVEL;
	if (exposeSessionEnvironment && ctx) {
		const model = ctx.model;
		env.LETTY_SESSION_ID = ctx.sessionManager.getSessionId();
		const sessionFile = ctx.sessionManager.getSessionFile();
		if (sessionFile) env.LETTY_SESSION_FILE = sessionFile;
		if (model) {
			env.LETTY_PROVIDER = model.provider;
			env.LETTY_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.LETTY_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current Letty session metadata as LETTY_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;

import type { Api, Model, ProviderHeaders } from "@letty/letty-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://letty.dev",
			"X-OpenRouter-Title": "letty",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Letty",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "letty-coding-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "letty" };
}

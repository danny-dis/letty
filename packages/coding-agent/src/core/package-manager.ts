...
import { type LettyManifest, readLettyManifest } from "./letty-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.LETTY_OFFLINE;
...
type SkillDiscoveryMode = "letty" | "agents";
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const shouldIncludeMarkdownFile =
				...
				!ig.ignores(relPath) &&
				((mode === "letty" && dir === root) || (mode === "agents" && dir !== root));
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "letty");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside letty and resolve letty APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @letty/letty-* peers. Stale auto-installed letty peers can otherwise block updates.
	...
	private getGitUpdateMarkerPath(targetDir: string): string {
		return join(dirname(targetDir), `.${basename(targetDir)}.letty-update-incomplete`);
	}
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "letty-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = readLettyManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof LettyManifest];
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			...
			// Project skills from .letty/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "letty"),
				projectMetadata,
		...
		// User extensions from ~/.letty/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.letty/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "letty"),
			userMetadata,

...
import * as builtinProviderCatalog from "@letty/letty-ai/providers/all";
...
/** Configured letty-ai Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			process.env.LETTY_OFFLINE === undefined,
		);
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		// Published letty-ai builds before ModelsStore returned void and accepted a provider ID.

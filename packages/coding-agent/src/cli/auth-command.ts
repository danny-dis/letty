import type { AuthResult } from "@letty/letty-ai";
...
export function printAuthCommandHelp(): void {
	console.log(`Usage:
  letty auth print-api-key [--provider <provider>] [--model <model>]
  letty auth print-bearer-token [--provider <provider>] [--model <model>] [--min-expiry <duration>]
  letty auth check [--provider <provider>] [--model <model>] [--json] [--credentials] [--no-refresh]

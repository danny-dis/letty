...
import { Markdown, type MarkdownTheme } from "@letty/letty-tui";
...
import { getLettyUserAgent } from "./utils/letty-user-agent.ts";
import { canonicalizePath, getCwdRelativePath } from "./utils/paths.ts";
import { formatVersionCheckError, getLatestLettyRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
const DEFAULT_INSTALLER_API_BASE = "https://letty.dev/api/installer/releases";
...
function getActiveManagedInstallRoot(): string | undefined {
	const configuredRoot = process.env.LETTY_MANAGED_INSTALL_ROOT?.trim();
	...
	// source checkout or another Letty installation launched from managed Letty as managed.
	...
	try {
		...
		if (marker.kind !== "letty-managed-install" || marker.schemaVersion !== 1 || marker.layout !== "releases-v1") {
			throw new Error();
		}
	} catch {
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getLettyUserAgent(VERSION) } });
	if (!response.ok) {
...
function verifyManagedRelease(releaseDir: string, expectedVersion: string): void {
	...
	if (result.error || result.status !== 0) {
		...
		throw new Error(`Could not verify managed Letty ${expectedVersion}: ${reason}`);
	}
	const installedVersion = result.stdout.trim();
	if (installedVersion !== expectedVersion) {
		throw new Error(`Managed Letty smoke test returned version ${installedVersion}; expected ${expectedVersion}.`);
	}
}
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		...
		if (error instanceof Error && "code" in error && error.code === "ELOCKED") {
			throw new Error("Another managed Letty update is already running.");
		}
		throw error;
	}
	...
	try {
		cleanupManagedStaging(managedRoot);
		const installerApiBase = (process.env.LETTY_INSTALLER_API_BASE?.trim() || DEFAULT_INSTALLER_API_BASE).replace(
			/\/+$/,
			"",
		);
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|letty] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update letty, installed packages, or model catalogs.

Options:
  --self                  Update letty only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update letty and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall letty even if the current version is latest

Short forms:
  ${APP_NAME} update                Update letty only
  ${APP_NAME} update --all          Update letty and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update letty             Update letty only (self works as alias to letty)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "letty";
			if (sourceIsSelf) {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestLettyRelease>>;
	try {
		latestRelease = await getLatestLettyRelease(VERSION, { retry: true });
	} catch (error: unknown) {

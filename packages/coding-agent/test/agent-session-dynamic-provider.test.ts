...
import { getModel } from "@letty/letty-ai/compat";
...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `letty-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(letty) => {
				letty.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
		...
		const session = await createSession([
			(letty) => {
				letty.on("session_start", () => {
					letty.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("registers native letty-ai providers during extension loading", async () => {
		const session = await createSession([
			(letty) => {
				letty.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-top-level"));
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(letty) => {
				letty.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						letty.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
	...
	it("registers native letty-ai providers at command time", async () => {
		const session = await createSession([
			(letty) => {
				letty.registerCommand("use-native", {
					description: "Use native provider",
					handler: async () => {
						letty.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);

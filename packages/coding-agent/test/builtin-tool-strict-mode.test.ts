...
describe("strict built-in tools", () => {
	...
	it.each([undefined, "0", "1"])("prefers strict sampling with LETTY_EXPERIMENTAL=%s", (experimental) => {
		vi.stubEnv("LETTY_EXPERIMENTAL", experimental);
	...
	it.each([{ activeTools: [] }, { activeTools: ["read"] }, { activeTools: [...strictToolNames] }])(
		...
		async ({ activeTools }) => {
			const cwd = mkdtempSync(join(tmpdir(), "letty-non-strict-tools-"));
				...
				extensionFactories: [
					(letty) => {
						letty.on("session_start", () => {
							...
							for (const name of strictToolNames) {
								letty.registerTool({ ...definitions[name], constrainedSampling: false });
							}
							letty.setActiveTools(activeTools);
						});
					},
				],
			});

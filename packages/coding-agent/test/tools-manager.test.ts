...
const originalOffline = process.env.LETTY_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.LETTY_OFFLINE;
	else process.env.LETTY_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});

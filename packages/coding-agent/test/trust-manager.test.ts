...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".letty", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".letty", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".letty", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".letty"), { recursive: true });
			writeFileSync(join(cwd, ".letty", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".letty"), { recursive: true, force: true });

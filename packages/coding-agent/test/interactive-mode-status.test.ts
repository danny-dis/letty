...
import { type AutocompleteProvider, CombinedAutocompleteProvider } from "@letty/letty-tui";
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.letty/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.letty/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.letty/extensions",
				}),
			},
			{
				path: "/tmp/project/.letty/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.letty/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.letty/extensions",
			...
			{
				path: "/tmp/project/.letty/npm/node_modules/letty-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.letty/npm/node_modules/letty-markdown-preview/extensions/index.ts",
					{
						source: "npm:letty-markdown-preview",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.letty/npm/node_modules/letty-markdown-preview",
					},
				),
			},
			{
				path: "/tmp/project/.letty/npm/node_modules/@scope/letty-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.letty/npm/node_modules/@scope/letty-scoped/extensions/index.ts",
					{
						source: "npm:@scope/letty-scoped",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.letty/npm/node_modules/@scope/letty-scoped",
					},
				),
			},
			{
				path: "/tmp/project/.letty/git/github.com/HazAT/letty-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.letty/git/github.com/HazAT/letty-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/letty-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.letty/git/github.com/HazAT/letty-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.letty/git/github.com/HazAT/letty-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.letty/git/github.com/HazAT/letty-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/letty-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  main.ts"`);
	...
	test("package extensions still strip index.ts correctly (regression guard)", () => {
		...
		expect(normalizeRenderedOutput(fakeThis.loadedResourcesContainer)).toMatchInlineSnapshot(`
"[Extensions]
  letty-markdown-preview"`);
	});

	test("labels npm sibling extensions relative to the declaring package", () => {
		const extensions: ExtensionFixture[] = [
			{
				path: "/tmp/project/.letty/npm/node_modules/primary-package/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.letty/npm/node_modules/primary-package/index.ts", {
					source: "npm:primary-package",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.letty/npm/node_modules/primary-package",
				}),
			},
			{
				path: "/tmp/project/.letty/npm/node_modules/sibling-package/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.letty/npm/node_modules/sibling-package/index.ts", {
					source: "npm:primary-package",
					scope: "project",
					origin: "package",
					baseDir: "/tmp/project/.letty/npm/node_modules/primary-package",
...
"[Extensions]
  primary-package, primary-package:../sibling-package"`);
	...
	test("labels Windows npm sibling extensions relative to the declaring package", () => {
		const primaryPath = "C:\\Users\\me\\.letty\\agent\\npm\\node_modules\\primary-package\\index.ts";
		const siblingPath = "C:\\Users\\me\\.letty\\agent\\npm\\node_modules\\sibling-package\\index.ts";
		const baseDir = "C:\\Users\\me\\.letty\\agent\\npm\\node_modules\\primary-package";
		const extensions: ExtensionFixture[] = [
			{
				path: primaryPath,
...
"[Extensions]
  project
    /tmp/project/.letty/extensions/answer.ts
    /tmp/project/.letty/extensions/local-index
    git:github.com/HazAT/letty-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/letty-scoped
      extensions
    npm:letty-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "letty-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".letty", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.letty/agent/AGENTS.md, AGENTS.md");
	...
	test("shows system prompt context paths before project context files", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			systemPromptSource: { path: path.join(cwd, ".letty", "SYSTEM.md") },
			appendSystemPromptSources: [{ path: path.join(cwd, ".letty", "APPEND_SYSTEM.md") }],
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain(".letty/SYSTEM.md, .letty/APPEND_SYSTEM.md, AGENTS.md");
	});
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.loadedResourcesContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.letty/agent/AGENTS.md");
		expect(output).toContain("~/Development/letty-mono/AGENTS.md");
		expect(output).not.toContain("~/.letty/agent/AGENTS.md, AGENTS.md");
	});

...
import { type Context, createFacetHost, defineFacet, defineService } from "@letty/chord";
import { BACKGROUND_CONTEXT } from "@letty/chord/context";
import { Client, ServerError as ClientServerError } from "@letty/letty-client";
import { createUnixTransportFactory } from "@letty/letty-client/unix";
import { afterEach, beforeEach, describe, expect, test, vi } from "vitest";
import { ExampleFacetService } from "../examples/plugins/letty-example-plugin/src/contract.ts";
...
beforeEach(async () => {
	agentDir = await mkdtemp(join("/tmp", "letty-experimental-agent-"));
	directories.add(agentDir);
	await configureExperimentalWorkerModel(agentDir);
	vi.stubEnv("LETTY_CODING_AGENT_DIR", agentDir);
...
describe("experimental durable server composition", () => {
	test("uses LETTY_SERVER_DIR and LETTY_SERVER_ID", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-server-dir-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("LETTY_SERVER_DIR", directory);
		vi.stubEnv("LETTY_SERVER_ID", serverId);
	...
	test("serializes concurrent cold activation and retires after both clients leave", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-auto-server-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("LETTY_SERVER_DIR", directory);
		vi.stubEnv("LETTY_SERVER_ID", serverId);
	...
	test("passes client plugin packages to a cold server and restores them for its next generation", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-auto-plugin-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		const packagePath = fileURLToPath(new URL("../examples/plugins/letty-example-plugin", import.meta.url));
		vi.stubEnv("LETTY_SERVER_DIR", directory);
		vi.stubEnv("LETTY_SERVER_ID", serverId);
		...
		try {
			...
			expect(loaded.facets.map(({ id }) => id)).toEqual(["@letty/letty-example-plugin/tui"]);
	...
	test("retires a cold server after its only Session attachment disconnects", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-auto-session-"));
		directories.add(directory);
		const serverId = "00000000-0000-4000-8000-000000000001";
		vi.stubEnv("LETTY_SERVER_DIR", directory);
		vi.stubEnv("LETTY_SERVER_ID", serverId);
	...
	test("runs and discovers multiple logical servers from one directory", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-multi-server-"));
		directories.add(directory);
	...
	test("loads conventional Session facets from multiple configured plugin packages", async () => {
		...
		await Promise.all([
			writeFile(
				...
				`${JSON.stringify({
					name: "@letty/second-session-plugin",
					version: "1.0.0",
					peerDependencies: { "@letty/chord": "^0.84.4" },
				})}\n`,
			),
			writeFile(
				join(secondPackagePath, "src", "session.ts"),
				'import { defineFacet, defineService } from "@letty/chord"; const Service = defineService("test.second-plugin"); export default defineFacet({ id: "second-session-plugin", setup(env) { env.provide(Service, { async read() { return "second"; } }); } });\n',
			),
		]);
		const runtime = await startServer({ ...sessionWorkerModel, directory });
		servers.add(runtime);
		...
		await activated.plugins.prepareSession(
			{
				sessionId: "demo-1",
				packagePaths: [
					fileURLToPath(new URL("../examples/plugins/letty-example-plugin", import.meta.url)),
					secondPackagePath,
				],
			},
			BACKGROUND_CONTEXT,
	...
	test("retires an unclaimed idle worker after replacement demand expires", async () => {
		const directory = await mkdtemp(join("/tmp", "letty-orphan-worker-"));
		directories.add(directory);
		vi.stubEnv("__LETTY_SESSION_WORKER_ORPHAN_DEMAND_GRACE_MS", "50");

...
import { fauxAssistantMessage, fauxToolCall, registerFauxProvider } from "@letty/letty-ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `letty-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(letty: ExtensionAPI) => {
						letty.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((letty: ExtensionAPI) => {
			letty.on("message_end", (event) => {
	...
	it("settles the active response before session replacement", async () => {
		...
		const { runtime, faux } = await createRuntimeForTest((letty: ExtensionAPI) => {
			letty.registerTool({
				name: "block",
				label: "Block",
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((letty: ExtensionAPI) => {
			letty.on("session_before_switch", (event) => {
				events.push(event);
			});
			letty.on("session_shutdown", (event) => {
				events.push(event);
			});
			letty.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `letty-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((letty: ExtensionAPI) => {
			letty.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(
			tmpdir(),
			`letty-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `letty-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `letty-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);

...
import type { AgentMessage, AgentTool } from "@letty/letty-agent-core";
import { Agent } from "@letty/letty-agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@letty/letty-ai/compat";
import { registerFauxProvider, streamSimple } from "@letty/letty-ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `letty-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);

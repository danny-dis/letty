import type { AgentTool, ThinkingLevel } from "@letty/letty-agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model, type Usage } from "@letty/letty-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model to the session and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(letty) => {
					letty.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(letty) => {
					letty.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
	...
	it("allows extension commands to inspect live system prompt options", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(letty) => {
					letty.registerCommand("inspect-options", {
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(letty) => {
					letty.on("session_start", async (event) => {
						lifecycleEvents.push(`start:${event.reason}`);
					});
					letty.on("session_shutdown", async (event) => {

import { type AssistantMessage, fauxAssistantMessage, fauxProvider } from "@letty/letty-ai";
import { getApiProvider } from "@letty/letty-ai/compat";
import { expect, it } from "vitest";
import { createHarness } from "../harness.ts";

// Regression for #8964: extensions can stream responses from providers registered with letty.registerProvider().
it.each(["stream", "streamSimple"] as const)(
	...
	async (method) => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(letty) => {
					letty.registerProvider(faux.provider.id, {
						api: faux.api,
				...
				(letty) => {
					letty.registerCommand("stream-custom", {

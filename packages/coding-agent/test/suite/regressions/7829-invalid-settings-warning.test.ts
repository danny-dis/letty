import { Container } from "@letty/letty-tui";
...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.LETTY_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.LETTY_OFFLINE;
			else process.env.LETTY_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});

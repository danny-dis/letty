...
describe("issue #8423 extension factory failure", () => {
	it("discards runtime changes and disables the failed API", async () => {
		...
		await loadExtensionFromFactory(
			(letty) => letty.registerProvider("working-provider", providerConfig),
			process.cwd(),
			eventBus,
			runtime,
			"<working>",
		...
		await expect(
			loadExtensionFromFactory(
				(letty) => {
					capturedApi = letty;
					letty.events.on("factory-failure", () => {
						eventCalls++;
					});
					letty.registerFlag("failed-flag", { type: "boolean", default: true });
					flagDuringLoad = letty.getFlag("failed-flag");
					letty.unregisterProvider("working-provider");
					letty.registerProvider("failed-provider", providerConfig);

...
describe("extension active tools next-turn refresh", () => {
	it("applies letty.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(letty) => {
				letty.registerTool({
					...
					execute: async () => {
						letty.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(letty) => {
				letty.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						letty.setActiveTools([...letty.getActiveTools(), "after_load"]);
						return {

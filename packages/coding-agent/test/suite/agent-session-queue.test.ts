...
import { fauxAssistantMessage, fauxToolCall } from "@letty/letty-ai";
import type { ExtensionAPI, InputEvent } from "@letty/letty-coding-agent";
			...
			extensionFactories: [
				(letty: ExtensionAPI) => {
					letty.on("agent_end", async () => {
						if (sent) return;
						sent = true;
						letty.sendUserMessage("conflict report", { deliverAs: "followUp" });
					});
				},
			],
		});

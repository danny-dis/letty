...
describe("package commands", () => {
	...
	let originalLettyPackageDir: string | undefined;
	...
	function prepareManagedInstall(
		...
		writeFileSync(
			...
			`${JSON.stringify({ kind: "letty-managed-install", schemaVersion: 1, layout: "releases-v1" })}\n`,
		);
...
const lettyPath = path.join(binDir, process.platform === "win32" ? "letty.cmd" : "letty");
fs.writeFileSync(
	lettyPath,
...
if (process.platform !== "win32") fs.chmodSync(lettyPath, 0o755);
`,
		);
		...
		vi.stubEnv("LETTY_INSTALLER_API_BASE", "https://example.test/api/installer/releases");
		vi.stubEnv("LETTY_MANAGED_INSTALL_ROOT", managedRoot);
		process.env.LETTY_PACKAGE_DIR = selfPackageDir;
	...
	function mockManagedUpdate(targetVersion: string): void {
		vi.stubGlobal(
			...
			vi.fn(async (input: string | URL | Request) => {
				...
				if (url === "https://letty.dev/api/latest-version") {
	...
	beforeEach(() => {
		allowNetwork();
		tempDir = join(tmpdir(), `letty-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalLettyPackageDir = process.env.LETTY_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".letty"), { recursive: true });
		writeFileSync(join(projectDir, ".letty", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(letty) => {
							letty.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".letty", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".letty"), { recursive: true });
		writeFileSync(join(projectDir, ".letty", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".letty", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".letty", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("letty install <source> [-l]");
	...
	it("cycles project package overrides in config local mode", async () => {
		...
		storage.withLock("global", () => JSON.stringify({ packages: ["npm:letty-tools"] }));
		const settingsManager = SettingsManager.fromStorage(storage, { projectTrusted: true });
		const resolvedPaths = extensionPaths(join(tempDir, "pkg"), "npm:letty-tools", "user", ["bar.ts"]);
		...
		expect(settingsManager.getProjectSettings().packages).toEqual([
			{ source: "npm:letty-tools", autoload: false, extensions: ["-extensions/bar.ts"] },
		]);
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "letty --help" or "letty install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: letty install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		const previousSkipVersionCheck = process.env.LETTY_SKIP_VERSION_CHECK;
		process.env.LETTY_SKIP_VERSION_CHECK = "1";
		...
		try {
			...
			expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
				`letty is already up to date (v${VERSION})`,
			);
			expect(errorSpy).not.toHaveBeenCalled();
			expect(process.exitCode).toBeUndefined();
		} finally {
			if (previousSkipVersionCheck === undefined) {
				delete process.env.LETTY_SKIP_VERSION_CHECK;
			} else {
				process.env.LETTY_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			}
		}
	});
	...
	it("retries a transient self-update version check", async () => {
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) delete process.env.LETTY_SKIP_VERSION_CHECK;
			else process.env.LETTY_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	});

	it("updates installer-managed Letty through a staged immutable release", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			`Updated letty from ${VERSION} to ${targetVersion}`,
		);
		expect(errorSpy).not.toHaveBeenCalled();
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("rejects a concurrent managed update", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).not.toContain("Updated letty from");
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Another managed Letty update is already running.",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("rejects forced managed reinstalls", async () => {
		...
		expect(existsSync(npmRecordPath)).toBe(false);
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Managed letty installations do not support --force",
		);
		expect(process.exitCode).toBe(1);
	});
	...
	it("keeps npm self-updates non-managed when the managed environment is inherited", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@letty", "letty-coding-agent");
		...
		vi.stubEnv("LETTY_MANAGED_INSTALL_ROOT", inheritedManagedRoot);
...
`,
		...
		try {
			...
			expect(stdout).toContain(`Updated letty from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated letty from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/letty" ? "@newer-scope/letty" : "@new-scope/letty";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@letty", "letty-coding-agent");
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain("Updated letty");
			expect(stderr).toContain("exited with code 23");
			expect(stderr).toContain("If pnpm reports missing package versions");
			expect(stderr).toContain("Run `pnpm store prune` and retry `letty update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:letty-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "letty-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:letty-formatter?");
			expect(stdout).not.toContain("Updated letty-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:letty-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}

...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(async () => {
		tempDir = join(tmpdir(), `letty-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(letty) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = letty;
			},
			(letty) => {
		...
		letty!.sendUserMessage("Steer from extension", { deliverAs: "steer" });

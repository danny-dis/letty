...
import { readFacetBundleManifest } from "@letty/chord/node";
...
describe("server-selected presentation facets", () => {
	...
	test("restores plugin package selections for later server generations", async () => {
		const directory = await mkdtemp("/tmp/letty-presentation-profile-");
	...
	test("builds conventional plugin entries into the server-owned plugin cache", async () => {
		const directory = await mkdtemp("/tmp/letty-presentation-package-");
		directories.add(directory);
		const serverId = randomUUID();
		const packagePath = join(directory, "letty-example-plugin");
		await mkdir(join(packagePath, "src"), { recursive: true });
		await writeFile(
			join(packagePath, "package.json"),
			`${JSON.stringify({
				name: "@letty/test-plugin",
				version: "1.0.0",
				peerDependencies: {
					"@letty/chord": "^0.84.4",
					"@letty/letty-coding-agent": "^0.84.4",
				},
			})}\n`,
		);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@letty/chord"; import { SlashCommands } from "@letty/letty-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-a", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		expect(plugin.manifestPath).toMatch(
			new RegExp(`/plugin-builds/${serverId}/letty-example-plugin-[a-f0-9]{12}/chord-facets\\.json$`, "u"),
		);
		expect(first[0]?.plugin).toEqual({ id: "@letty/test-plugin", version: "1.0.0" });
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@letty/chord"; import { SlashCommands } from "@letty/letty-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-b", setup(env) { env.use(SlashCommands); } });\n',
		);
		...
		await Promise.all([
			...
			writeFile(
				...
				'import { defineFacet } from "@letty/chord"; export default defineFacet({ id: "second-built", setup() {} });\n',
			),
		]);
		...
		await writeFile(
			sourcePath,
			'import { defineFacet } from "@letty/chord"; import { SlashCommands } from "@letty/letty-coding-agent/experimental/plugin"; export default defineFacet({ id: "built-c", setup(env) { env.use(SlashCommands); } });\n',
		);
	...
	test("builds the example plugin package without a package-owned build script", async () => {
		const directory = await mkdtemp("/tmp/letty-example-plugin-");
		directories.add(directory);
		const serverId = randomUUID();
		...
		expect(manifest.plugin).toEqual({ id: "@letty/letty-example-plugin", version: "1.0.0" });

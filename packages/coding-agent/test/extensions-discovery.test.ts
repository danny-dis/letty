...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "letty-ext-test-"));
	...
	const extensionCode = `
		export default function(letty) {
			letty.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("loads the coding-agent entrypoint without rewriting letty-ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "@letty/letty-coding-agent";
				void getAgentDir;
				export default function(letty) {
					letty.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only letty-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@letty/letty-ai/oauth";
				void oauth;
				export default function(letty) {
					letty.registerCommand("test", { handler: async () => {} });
				}
	...
	it("discovers subdirectory with package.json letty field", async () => {
	...
	it("keeps package.json letty extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with letty field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without letty field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ letty: { extensions: ["./entry.ts"] } }));
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(letty) {
				letty.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
				letty.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				letty.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(letty) {
				letty.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(letty) {
				letty.on("agent_start", async () => {});
				letty.on("tool_call", async (event) => undefined);
				letty.on("agent_end", async () => {});
			}
		`;

...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const extensionFile = join(cachedDir, "letty-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "letty-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ letty: { extensions: ["./letty-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "letty-extensions/session-breakdown.ts")).toBe("// fresh");
		});
	});
});

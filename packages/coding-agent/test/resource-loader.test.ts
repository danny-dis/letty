...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "letty-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".letty", "prompts");
			...
			const projectSkillDir = join(cwd, ".letty", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".letty", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".letty", "themes"), { recursive: true });
	...
	letty.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".letty"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".letty", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".letty", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".letty", "extensions");
			mkdirSync(userExtDir, { recursive: true });
			mkdirSync(projectExtDir, { recursive: true });
			const loadCountKey = `__lettyTrustPreloadCount_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export default function(letty) {
	...
	letty.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(letty) {
	letty.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".letty", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
	...
	letty.registerCommand("project-only", {
	...
	letty.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.letty", async () => {
			const projectDir = join(cwd, ".letty");
			mkdirSync(projectDir, { recursive: true });
			writeFileSync(join(projectDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should skip project resources that require trust when project is not trusted", async () => {
			...
			const skillDir = join(projectDir, "skills", "project-skill");
			...
			mkdirSync(promptsDir, { recursive: true });
			mkdirSync(themesDir, { recursive: true });
			writeFileSync(join(projectDir, "SYSTEM.md"), "Project system prompt.");
...
Project skill content`,
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const projectDir = join(cwd, ".letty");
			mkdirSync(projectDir, { recursive: true });
			writeFileSync(join(projectDir, "APPEND_SYSTEM.md"), "Additional instructions.");

...
describe("experimental CLI commands", () => {
	...
	test("parses client transport addresses", () => {
		expect(cli.parse(["client", "--connect", "unix:///tmp/letty.sock"])).toEqual({
			ok: true,
			command: { command: "client", connect: { transport: "unix", path: "/tmp/letty.sock" } },
		});
		expect(
	...
	test.each([
		[["client", "--listen", "unix:///tmp/letty.sock"], UNSUPPORTED_CLIENT_OPTIONS],
		[["server", "--listen", "unix:///tmp/letty.sock"], UNSUPPORTED_SERVER_OPTIONS],
		[["server", "--connect", "unix:///tmp/letty.sock"], UNSUPPORTED_SERVER_OPTIONS],
		...
		[["client", "--connect", "unix:///tmp/letty.sock?wrong=value"], "Invalid --connect address"],

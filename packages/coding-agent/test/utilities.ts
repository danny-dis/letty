...
import type { OAuthCredentials } from "@letty/letty-ai";
import { getModel, streamSimple } from "@letty/letty-ai/compat";
import { builtinProviders } from "@letty/letty-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.letty/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".letty", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.letty/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.letty/agent/auth.json
 */
...
/** Path to the real letty agent config directory */
export const LETTY_AGENT_DIR = join(homedir(), ".letty", "agent");

/**
 * Get an AuthStorage instance backed by ~/.letty/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `letty-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);

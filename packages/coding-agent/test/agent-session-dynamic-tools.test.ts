...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `letty-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(session.systemPrompt).toContain(
			"You can inspect LETTY_* environment variables for current model and session details.",
		);
		await bashTool.execute("bash-env", { command: "printf ok" });
		expect(sessionEnv).toMatchObject({
			LETTY_SESSION_ID: session.sessionId,
			LETTY_SESSION_FILE: session.sessionFile,
			LETTY_PROVIDER: model.provider,
			LETTY_MODEL: model.id,
			LETTY_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("LETTY_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("LETTY_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("LETTY_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("LETTY_MODEL");
		expect(optedOutEnv).not.toHaveProperty("LETTY_REASONING_LEVEL");

...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@letty/letty-ai/compat";
import { getApiProvider, getModels, getSupportedThinkingLevels } from "@letty/letty-ai/compat";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `letty-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	function getOpenRouterOpusModelId(): string {
		const model = getModels("openrouter").find(
			(m) => m.id.startsWith("anthropic/claude-opus-") && !m.id.includes(":"),
		);
		if (!model) throw new Error("Expected an Anthropic Opus model in the generated OpenRouter catalog");
		return model.id;
	}
	...
	describe("modelOverrides (per-model customization)", () => {
		test("model override applies to a single built-in model", async () => {
			...
			const opus = models.find((m) => m.id === getOpenRouterOpusModelId());
		...
		test("supportsFinishReason can be configured at provider and model levels", async () => {
			...
			const opus = models.find((model) => model.id === getOpenRouterOpusModelId());

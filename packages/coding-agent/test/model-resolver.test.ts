...
import type { Model } from "@letty/letty-ai";
import { getModel, streamSimple } from "@letty/letty-ai/compat";
import { getBuiltinModels, getBuiltinProviders } from "@letty/letty-ai/providers/all";
...
describe("default model selection", () => {
	...
	describe("persisted default model scoping", () => {
		...
		async function createSession(options: { scoped: boolean; persistedScope?: string[] }) {
			const tempDir = join(tmpdir(), `letty-default-scope-${Date.now()}-${Math.random().toString(36).slice(2)}`);

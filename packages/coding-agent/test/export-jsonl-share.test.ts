...
import type { AssistantMessage, ToolResultMessage } from "@letty/letty-ai/compat";
...
describe("JSONL share export", () => {
	...
	it("adds presentation data without changing conversation IDs or links", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "letty-jsonl-share-"));
		...
		try {
			...
			expect(normalRecords.some((record) => record.type === "custom" && record.customType === "letty.share")).toBe(
				false,
			);

...
import { InMemoryModelsStore } from "@letty/letty-ai";
...
beforeEach(() => {
	allowNetwork();
	tempDir = join(tmpdir(), `letty-test-radius-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("Radius provider", () => {
	it("restores the legacy credential catalog without network access", async () => {
		...
		expect(model).toMatchObject({ api: "letty-messages", baseUrl: "https://radius.example.com/v1" });
	...
	it("fetches and stores the catalog for configured Radius auth", async () => {
		...
		const radiusRequest = vi
			.mocked(fetch)
			.mock.calls.find(([url]) => String(url) === "https://radius.letty.dev/v1/config");
	...
	it("does not fetch or expose Radius models without configured auth", async () => {
		...
		expect(fetchSpy.mock.calls.some(([url]) => String(url).includes("radius.letty.dev/v1/config"))).toBe(false);
	});

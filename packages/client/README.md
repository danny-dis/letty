...
Transport-neutral client for the experimental Letty service protocol.

```ts
import { Client, type ByteTransportFactory } from "@letty/letty-client";
...
```ts
import { Client } from "@letty/letty-client";
import { createUnixTransportFactory } from "@letty/letty-client/unix";

const client = new Client({
  serverId: "01234567-89ab-4def-8123-456789abcdef",
  transportFactory: createUnixTransportFactory({ path: "/tmp/letty.sock" }),
});
...
```ts
import { discoverUnixServers } from "@letty/letty-client/unix";

const routes = await discoverUnixServers({ directory: "/run/user/1000/letty" });
// [{ serverId: "...", path: "/run/user/1000/letty/<serverId>.sock" }]
```

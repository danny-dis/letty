...
import { parseServiceCall } from "@letty/chord";
import { ClientMessageDecoder, encodeServerMessage, PROTOCOL_VERSION } from "@letty/letty-protocol";
...
async function makeSocketPath(): Promise<string> {
	const directory = await mkdtemp(join("/tmp", "letty-client-transport-"));
	tempDirectories.add(directory);
	return join(directory, "letty.sock");
}
...
test("rejects invalid Unix transport options", () => {
	...
	expect(() => createUnixTransportFactory({ path: "/tmp/letty.sock", maxPendingBytes: 0 })).toThrow(/positive/);
});

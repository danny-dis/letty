...
class FailingSpillExecutionEnv extends NodeExecutionEnv {
	override async createTempFile(
		...
		if (options?.prefix === "letty-output-") {
...
describe("NodeExecutionEnv", () => {
	...
	it("expands home-relative paths and file URLs", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
		expect(getOrThrow(await env.absolutePath("~/letty-node-env-test", BACKGROUND_CONTEXT))).toBe(
			join(homedir(), "letty-node-env-test"),
		);
	...
	it.each([
		...
		["an empty override shadows the base value", { LETTY_SESSION_FILE: "" }, "x:"],
		[
			"a string override replaces the base value",
			{ LETTY_SESSION_FILE: "/sessions/current.jsonl" },
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					LETTY_SESSION_FILE: "/stale/parent.jsonl",
					LETTY_CODING_AGENT: "true",
					LETTY_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const collected = await collectShellOutput(
				env,
				`printf '%s:%s|%s|%s' "\${LETTY_SESSION_FILE+x}" "\${LETTY_SESSION_FILE-}" "$LETTY_CODING_AGENT" "$LETTY_NODE_ENV_PRESERVED_TEST"`,
				{ env: overrides },
				BACKGROUND_CONTEXT,
			);
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "LETTY_NODE_ENV_INHERITED_TEST";
		const configuredKey = "LETTY_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "LETTY_NODE_ENV_EXPLICIT_TEST";
	...
	// Regression test for https://github.com/letty/letty/issues/8992

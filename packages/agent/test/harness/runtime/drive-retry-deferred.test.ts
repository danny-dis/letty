...
describe("runtime deferred polling", () => {
	...
	it("replaces an unknown poll under fresh ids at the same poll number and deletes old frames", async () => {
			...
			.find((writes) =>
				writes.some(
					(write) =>
						write.kind === "list" &&
						write.op === "delete" &&
						write.namespace === "letty.pending.assistant_frame" &&
		...
		const intentState = intent?.find(
			(write) => write.kind === "value" && write.op === "set" && write.namespace === "letty.op.state",
		);
	...
	it("abandons unknown ids and frames into configuration failure when the captured model is unavailable", async () => {
		...
		expect(
			...
			"value:delete:letty.op.meta",
			"value:delete:letty.op.state",
			"list:delete:letty.pending.assistant_frame",
			"value:set:letty.result",
			"value:set:letty.lane.state",
		]);
		expect(
			fixture.storage
				.getCommitAttempts()

...
class ObservedMemoryStorage extends MemoryStorage {
	...
	override async commit(writes: Write[], context: Context) {
		...
		if (
			writes.some(
				(write) => write.kind === "value" && write.op === "set" && write.namespace === "letty.op.tool_args",
			)
		) {
			this.observations.push("intent_commit");
		}
		const stagesOutcome = writes.some(
			(write) => write.kind === "value" && write.op === "set" && write.namespace === "letty.pending.entry",
		);
		...
		if (
			...
			writes.some(
				(write) =>
					write.kind === "value" && write.op === "delete" && write.namespace === "letty.pending.tool_output",
			)
		) {

import { createModels, fauxAssistantMessage, fauxProvider, type MutableModels } from "@letty/letty-ai";
...
describe("runtime structural drive", () => {
	...
	it("gives each split-turn provider request its own durable intent and usage row", async () => {
		...
		const requestIndices = attempts.flatMap((writes) =>
			writes.flatMap((write) =>
				write.kind === "value" &&
				write.op === "set" &&
				write.namespace === "letty.op.state" &&
		...
		expect(
			attempts.flat().some((write) => write.kind === "list" && write.namespace === "letty.pending.assistant_frame"),
		).toBe(false);
		expect(
	...
	it("moves an unsummarized navigation and cleans up in one terminal transaction", async () => {
		...
		expect(
			writes.some(
				(write) => write.kind === "value" && write.namespace === "letty.op.state" && write.op === "delete",
			),
		).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "letty.result")).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "letty.lane.state")).toBe(true);
	});

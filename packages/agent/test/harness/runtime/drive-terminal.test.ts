...
describe("runtime terminal cleanup mechanics", () => {
	it("deletes every operation-owned family and exact live frame list but preserves the lane inbox", async () => {
		...
		expect(writes.map(address)).toEqual([
			"value:delete:letty.op.meta:run",
			"value:delete:letty.op.state:run",
			"value:delete:letty.op.tool_args:run:step:0",
			"value:delete:letty.op.tool_memo:run:invocation:memo",
			"value:delete:letty.op.preparation:run:task",
			"value:delete:letty.pending.tool_output:run:invocation",
			"list:delete:letty.pending.assistant_frame:run:response",
		]);
	...
	it("deletes staged tool outcomes and leaves completed results alone", async () => {
		...
		expect(writes.map(address)).toContain("value:delete:letty.pending.entry:staged");
		expect(writes.map(address)).not.toContain("value:delete:letty.pending.entry:placed");
	});
	...
	it.each([
		...
		expect(writes.map(address)).toEqual([
			`value:delete:letty.op.meta:${operationId}`,
			`value:delete:letty.op.state:${operationId}`,
			`value:delete:letty.op.tool_args:${operationId}:step:0`,
			`value:delete:letty.op.tool_memo:${operationId}:invocation:memo`,
			`value:delete:letty.op.preparation:${operationId}:task`,
			`value:delete:letty.pending.tool_output:${operationId}:invocation`,
		]);
	});
});

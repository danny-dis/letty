...
describe("bound value addresses", () => {
	it("binds immutable scalar and list addresses with validated components", () => {
		...
		expect(value<unknown>("letty.application")).toEqual({ kind: "value", namespace: "letty.application", key: "" });
	});
...
describe("built-in durable addresses", () => {
	it("uses the exact reserved namespaces, keys, kinds, and value types", () => {
		...
		expect([
			...
			{ kind: "value", namespace: "letty.branch.tip", key: "review" },
			{ kind: "value", namespace: "letty.lane.config", key: "review" },
			{ kind: "value", namespace: "letty.lane.state", key: "review" },
			{ kind: "value", namespace: "letty.result", key: "operation" },
			{ kind: "value", namespace: "letty.op.meta", key: "operation" },
			{ kind: "value", namespace: "letty.op.state", key: "operation" },
			{ kind: "value", namespace: "letty.op.tool_args", key: "operation:step:2" },
			{ kind: "value", namespace: "letty.op.tool_memo", key: "operation:invocation:name" },
			{ kind: "value", namespace: "letty.op.preparation", key: "operation:task" },
			{ kind: "value", namespace: "letty.pending.entry", key: "entry" },
			{ kind: "value", namespace: "letty.pending.tool_output", key: "operation:invocation" },
			{ kind: "list", namespace: "letty.pending.assistant_frame", key: "operation:response" },
			{ kind: "value", namespace: "letty.session.name", key: "" },
			{ kind: "value", namespace: "letty.entry.label", key: "entry" },
		]);
	});
	...
	it("exports exactly the five documented scan-prefix constructors", () => {
		...
		expect(prefixes).toEqual([
			{ kind: "value", namespace: "letty.branch.tip", key: "" },
			{ kind: "value", namespace: "letty.op.tool_args", key: "operation:" },
			{ kind: "value", namespace: "letty.op.tool_memo", key: "operation:invocation:" },
			{ kind: "value", namespace: "letty.op.preparation", key: "operation:" },
			{ kind: "value", namespace: "letty.pending.tool_output", key: "operation:" },
		]);

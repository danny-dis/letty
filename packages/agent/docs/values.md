...
await session.readList(workspaceEvents("letty"), { limit: 100 }, context);
```
...
- namespace `letty` and every `letty.*` namespace are reserved for built-ins by contract;
...
export const branchTip = (lane: string) =>
  value<string | null>("letty.branch.tip", lane);

export const operationState = (operationId: string) =>
  value<OperationState>("letty.op.state", operationId);
...
export const laneConfig = (lane: string) =>
  value<LaneConfiguration>("letty.lane.config", lane);
export const laneState = (lane: string) =>
  value<LaneState>("letty.lane.state", lane);
export const operationResult = (operationId: string) =>
  value<OperationResultRecord>("letty.result", operationId);

/** Used only by scanValues() to enumerate Branch names. */
export const branchTipInventoryPrefix = () =>
  value<string | null>("letty.branch.tip");

export const operationMeta = (operationId: string) =>
  value<OperationMeta>("letty.op.meta", operationId);
export const operationState = (operationId: string) =>
  value<OperationState>("letty.op.state", operationId);
...
export const operationPreparationPrefix = (operationId: string) =>
  value<DurableStructuralPreparation>("letty.op.preparation", `${operationId}:`);

export const pendingEntry = (entryId: string) =>
  value<PendingEntry>("letty.pending.entry", entryId);
export const pendingToolOutput = (operationId: string, invocationId: string) =>
  value<AgentToolResult<unknown>>(
    "letty.pending.tool_output",
...
export const pendingToolOutputPrefix = (operationId: string) =>
  value<AgentToolResult<unknown>>("letty.pending.tool_output", `${operationId}:`);

export const sessionName = value<string>("letty.session.name");
export const entryLabel = (entryId: string) => value<string>("letty.entry.label", entryId);
```

`OperationMeta` is immutable acceptance metadata stored at `letty.op.meta`. The process-local `Operation` projection is `{ meta: OperationMeta, state: OperationState }`, assembled from the separate metadata and state values; it is never stored at one address.
...
`AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` come from `@letty/letty-ai`. Do not define a second frame codec or reducer.
...
```jsonl
{"kind":"list","op":"append","seq":41,"namespace":"letty.pending.assistant_frame","key":"O:R","value":{"type":"text_delta","contentIndex":0,"delta":"hi"}}
{"kind":"list","op":"delete","seq":52,"namespace":"letty.pending.assistant_frame","key":"O:R"}
```
...
- operation-owned `letty.op.*` scalar values are not copied into an idle fork;
- immutable `letty.result` operation records are not copied by forks;
- `letty.pending.entry`, `letty.pending.tool_output`, and `letty.pending.assistant_frame` values/lists are not copied;
...
3. Namespace `letty` and every `letty.*` are reserved by contract; every built-in namespace starts with `letty.`, and application use is a trusted-programming defect.
...
- built-in address constructors produce exact `letty.branch.tip`, `letty.lane.*`, `letty.op.*`, `letty.pending.*`, `letty.session.name`, and `letty.entry.label` namespace/key/kind triples;
- every built-in namespace starts with `letty.`, while application fixtures use non-reserved namespaces;
- `branchTipInventoryPrefix()` binds the empty-key `letty.branch.tip` inventory prefix and is used only to enumerate Branches through `scanValues`;

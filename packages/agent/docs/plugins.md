...
`@letty/chord`. This document specifies how Letty composes that runtime with Letty-owned
...
→ @letty/providers-builtin:session provides letty.models/singleton

env.use(Models)
→ @letty/model-selection:tui requires letty.models/singleton

env.provideMany(QuestionDialogs)
→ @letty/question:session provides letty.question-dialog/keyed

env.observe(QuestionDialogs, handler)
→ @letty/question:tui requires letty.question-dialog/keyed
```
...
export const Models = defineService<Models>("letty.models");
```
...
const Agent = defineService<AgentFacetScope>("letty.local.agent", { local: true });
const Providers = defineService<ProviderContributionRegistry>("letty.local.providers", { local: true });
const Tools = defineService<ToolContributionRegistry>("letty.local.tools", { local: true });
```
...
const Tui = defineService<TuiHost>("letty.local.tui", { local: true });
```
...
export function createAgentControllerRuntimeFacet(lane: AgentLane) {
	return defineFacet({
		id: "@letty/agent-controller-runtime",
		setup(env) {
...
const Accounts = defineService<Accounts>("letty.accounts");
```
...
```text
fresh ProviderDraft
→ built-in provider contribution        (@letty/providers-builtin)
→ remote catalogue contribution         (@letty/providers-catalog)
→ models.json transformation            (@letty/providers-models-json)
→ authentication/availability marking   (@letty/auth)
...
const Fleet = defineService<FleetFacetScope>("letty.local.fleet", { local: true });
```
...
export const SessionDirectory = defineService<SessionDirectory>("letty.session-directory");
...
export const SessionManagement = defineService<SessionManagement>("letty.session-management");
```
...
const QuestionDialogs = defineService<QuestionDialogs>("letty.question-dialog");
```
		...
		tools.add((draft) => {
			draft.set("question", {
				...
				async execute(_toolCallId, params, _onUpdate, _toolContext, invocation, context) {
					...
					const memoName = "letty.question.answer";
...
const DiffReviewManager = defineService<DiffReviewManager>("letty.diff-review-manager");
const DiffReviews = defineService<DiffReviews>("letty.diff-review");
```

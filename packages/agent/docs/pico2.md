...
Assumed knowledge: letty-ai (`Models`, `streamSimple`, tool definitions, thinking levels, deferred
...
AssistantMessageFrame, DeferredHandle, Usage and Models reuse letty-ai; AgentMessage/ThinkingLevel and
...
// usage(task, conv, usage) appends to the main session list letty.usage; no special storage verb
```
...
```ts
const model = conversationValue<ModelIdentity>("letty.model");
const thinkingLevel = conversationValue<ThinkingLevel>("letty.thinking");
const activeTools = conversationValue<string[]>("letty.active-tools");
const planMode = conversationValue<boolean>("my-plugin.plan-mode");
const moves = conversationList<Move>("my-game.moves");

const sessionName = sessionValue<string>("letty.session.name");
const entryLabel = (id: number) => sessionValue<string>("letty.entry.label", String(id));
const requestInput = (requestId: string) => sessionValue<number>("letty.request-input", requestId);
...
(scope, kind, namespace, key) tuples name the same address within the receiver's scope. `letty` and
`letty.*` are reserved for core. Constructing the same address with incompatible value types is a
...
const frames = list<AssistantMessageFrame>("letty.pending.frames", "", work);
const memo = value<JsonValue>("letty.pending.memo", "step", work);
...
Letty/application concerns, not Chord concepts.
...
```ts
const model = conversationValue<ModelIdentity>("letty.model");
const thinking = conversationValue<ThinkingLevel>("letty.thinking");
const tools = conversationValue<string[]>("letty.active-tools");
...
**Telemetry** uses letty's callback `TelemetryContext` (typed schemas, no second contract). Spans and

...
export const pendingToolOutput = (operationId: string, invocationId: string) =>
  list<WireOp[]>("letty.pending.tool_output", `${operationId}:${invocationId}`, operationId);

export const pendingAssistantOutput = (operationId: string, entryId: string) =>
  list<WireOp[]>("letty.pending.assistant_output", `${operationId}:${entryId}`, operationId);

export const operationToolMemo = (operationId: string, invocationId: string, name: string) =>
  value<JsonValue>("letty.op.tool_memo", `${operationId}:${invocationId}:${name}`, operationId);

// unchanged — no scopeId, so Session by inference
export const laneStateValue = (lane: string) => value<DurableLaneState>("letty.lane.state", lane);
```
...
Scoping everything under `letty.op.*` on the grounds that it dies with the operation
...
| | addresses |
| --- | --- |
| **ephemeral (sidecar)** | `letty.pending.tool_output`, `letty.pending.assistant_output`, `letty.op.tool_memo` |
| **session (main log)** | `letty.lane.state`, `letty.op.state`, `letty.op.meta`, `letty.result`, `letty.pending.entry`, `letty.branch.tip`, `letty.op.tool_args`, `letty.op.preparation` |
...
```jsonc
// today — 1250 bytes
{"kind":"value","op":"set","seq":7,"namespace":"letty.op.state","key":"01a04cf6-…","value":{"at":"starting","control":{…},"settings":{…},"latestAssistantEntryId":null}}
{"kind":"value","op":"set","seq":8,"namespace":"letty.lane.state","key":"main","value":{"currentOperationId":"01a04cf6-…",…}}
{"kind":"value","op":"set","seq":9,"namespace":"letty.op.state","key":"01a04cf6-…","value":{"at":"checkpoint",…}}
{"kind":"value","op":"set","seq":12,"namespace":"letty.op.state","key":"01a04cf6-…","value":{"at":"assistant.ready",…}}
```

```jsonc
// with both dictionaries — 547 bytes
["@",0,"letty.op.state","01a04cf6-…"]
["v",0,7,[["r",{"at":"starting","control":{…},"settings":{…},"latestAssistantEntryId":null}]]]
["@",1,"letty.lane.state","main"]

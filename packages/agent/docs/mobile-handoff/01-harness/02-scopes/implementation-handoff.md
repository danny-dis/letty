...
All other current built-ins remain session-scoped. In particular, `letty.op.state`, `letty.op.meta`, `letty.op.tool_args`, `letty.op.preparation`, and `letty.pending.entry` remain in the main scope because their writes coordinate atomically with lane/operation state.

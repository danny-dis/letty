...
export const SessionStatusReporting =
  defineService<SessionStatusReporting>("letty.session-status-reporting");

export interface SessionStatusView {
  readonly state: State<Record<string, SessionStatus>>;
}
export const SessionStatusView =
  defineService<SessionStatusView>("letty.session-status");
```
...
| invocation | route |
| --- | --- |
| `letty` (bare, in a directory) | ask server to create a session for cwd |
| `letty --resume` | invoke the picker command at startup |
| `letty --session <id>` | attach directly |
...
```ts
// contract.ts
export const TranscriptState = defineState<TranscriptTail>("letty.transcript.tail");
```
...
```ts
export const Lane = defineKeyedService<LaneView>("letty.lane");   // key = lane name

export const laneSession = defineFacet({
  id: "@letty/lane:session",
  uses:     [Harness],
  provides: [Lane],
  observes: [],
...
export const BrowseState = defineState<BrowseView>("letty.browse.view");
...
export const FileBrowsing = definePeerService<FileBrowsing>("letty.file-browsing");
```
...
export const ConnectedPeers =
  defineService<{ readonly state: State<Record<PeerId, Principal>> }>("letty.peers");
```
...
const Tui = defineService<TuiHost>("letty.local.tui", { rpc: false });
```
...
```text
fresh working copy
→ built-in providers      (@letty/providers-builtin)
→ remote catalogue        (@letty/providers-catalog)
→ models.json transform   (@letty/providers-models-json)
→ auth/availability mark  (@letty/auth)
→ validated state
```
...
```ts
import { object, array, string, int, oneOf, literal } from "@letty/schema";
...
`@letty/schema` re-exports TypeBox constructors as free functions so declarations stay
...
```
POST /v1/call/letty.transcript/page
...
```
GET /v1/state/letty.lane/main/snapshot

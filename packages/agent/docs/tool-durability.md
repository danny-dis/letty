...
A complete finalized result becomes durable immediately in `letty.pending.entry`; the call becomes `outcome_ready`; placement happens later when every earlier source position is complete or ready.
...
The mutation verifies the same operation, turn, source position, and invocation remain `effect_pending`. It does not rewrite `letty.op.state`. A late checkpoint after settlement returns without committing.
...
3. Every `outcome_ready` call has exactly one complete matching `letty.pending.entry` value.
...
```text
operationToolMemo(operationId, invocationId, name) → value("letty.op.tool_memo", ...)
pendingToolOutput(operationId, invocationId)      → value("letty.pending.tool_output", ...)
pendingEntry(resultEntryId)                       → value("letty.pending.entry", ...)
```

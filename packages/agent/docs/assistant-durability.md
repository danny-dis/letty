...
- `AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` from `@letty/letty-ai`;
...
4. Reuse letty-ai's canonical frame conversion and reduction semantics.
...
Create one letty-ai encoder per provider stream and feed it every event in order:
...
Do not define a second harness frame codec or reducer. Persistence stores the exported letty-ai value directly; hydration calls `reduceAssistantMessageFrames()`.
...
Idle forks never copy lists in the `letty.pending.assistant_frame` address family. Precise rewrites and migrations page frame lists and preserve element sequences when retaining them.
...
3. Every stored element is an exported letty-ai `AssistantMessageFrame`.

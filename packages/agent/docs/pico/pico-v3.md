...
`handoff` stores its message and starts a context; `system` stores the exact letty-ai `SystemMessage`
...
assistant cut before all its results, request-local letty-ai transformation supplies missing results;
...
const generationKind = defineTaskKind({
  kind: "generation",
  config: {
    model:         conversationValue<ModelId>("letty.model", { rewind: true }),
    thinking:      conversationValue<ThinkingLevel>("letty.thinking", { rewind: true }),
    selectedTools: conversationValue<string[]>("letty.tools.selected", { rewind: true }),
    profile:       conversationValue<string>("letty.prompt.profile", { rewind: true }),
    budgetMs:      conversationValue<number>("letty.tool.budget", { rewind: false }),
  },
  ...
});
...
execute (`ctx.sleep`, which throws on abort). Not everything on the signal throws: letty-ai's provider stream
...
The inbox is a conversation-scoped sticky list, `letty.inbox`. A queued item is the message itself;
...
not zero. The record is a session list, `letty.usage`, one element per attempt (conversation, task,
model, tokens, cost), appended in the settlement commit together with an update of the session
value `letty.usage.totals`, so stats are a point read and never a fold. Tools and jobs append the same
...
<harness>
[warn] output truncated: 2,000 of 51,204 lines shown; full output at /tmp/letty/out-4421.log
</harness>
```
...
interface SessionView {
  ...
  readonly values: ReadonlyMap<Address, JsonValue>;     // session-scoped: letty.usage.totals, letty.session.name, ...
  readonly faulted: boolean;
  readonly readAt: Id;
}
type SessionEvent =

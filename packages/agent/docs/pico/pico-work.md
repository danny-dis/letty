...
transcript order, stored model arrays concatenated, then letty-ai tool results ordered by call index.
...
`letty.inbox` as a conversation list, the modes, the three dequeue points, `cancelQueued`, abort
...
`report` and `usage`, the usage ledger (`letty.usage` + totals).
...
- `@letty/chord/delta` (15, 16, 17)
- `@letty/letty-ai`: `faux` provider for tests, `utils/estimate` for thresholds, `SystemMessage` (4, 9, 12)

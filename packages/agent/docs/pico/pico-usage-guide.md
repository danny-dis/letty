...
Durable agent harness for letty: one session file, any number of conversations, every piece of work
...
```bash
npm install @letty/letty-agent
```

## Quick Start

```typescript
import { Harness, JsonlStorage } from '@letty/letty-agent';
import { readTool, writeTool, bashTool } from '@letty/letty-agent/tools';
...
| option | what it is |
|---|---|
| `models` | a letty-ai `Models` collection |
...
```typescript
import { generationKind } from '@letty/letty-agent/kinds';
...
// {
//   model:         conversationValue<ModelRef>('letty.model', { rewind: true }),
...
letty-ai models the system prompt as messages in the transcript: the first rendered prompt of a
...
with native mid-conversation system messages the entry goes out as one; elsewhere letty-ai renders it
...
const name     = sessionValue<string>('letty.session.name');
...
```typescript
import { Type, type Tool } from '@letty/letty-agent';

...
boundary. Letty owns the concrete wire envelope, routing, attachment state, and error adapters
...
const SessionDirectory = defineService<SessionDirectory>("letty.session-directory");
const SessionManagement = defineService<SessionManagement>("letty.session-management");
```

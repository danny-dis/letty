...
**Both scopes**: copy `letty.session.name`; copy `letty.entry.label` values only for copied entries; exclude the usage ledger, `letty.result`, all `letty.op.*`, all `letty.pending.*` (entries, tool checkpoints, assistant frames), and every trace of open-operation state. Destination usage totals start at zero; destination `messageCount` equals the number of copied message entries, exactly as current conformance already proves. Destination metadata records `parentSessionId = source.id`. Sequence preservation is not part of the fork contract: backends may allocate destination-local sequences for copied and transformed writes, provided destination sequence and list-cursor semantics remain internally valid.

**Application values/lists (outside the reserved `letty`/`letty.*` namespaces)**: tree scope copies every current scalar value and every surviving list element; branch scope copies none. A `seq <= tipSeq` cutoff is forbidden as "historical reconstruction" — it is provably not one:
...
- `letty.op.*`, `letty.pending.*`, `letty.result` → exclude, both scopes.
- `letty.session.name` → copy.
- `letty.entry.label` → copy iff the keyed entry is copied.
- `letty.branch.tip`, `letty.lane.config`, `letty.lane.state` → structured lane actions; the scope-specific rules (branch keeps only the named lane and rewrites its tip; lane state is always replaced with fresh idle state; tree keeps all) live in one shared driver consumed by all backends.
- The exact namespace `letty` and any other `letty.*` namespace → the fork **fails**, but only when current surviving state exists at fork time (a current scalar row or a surviving list element). Historical writes later replaced or deleted are absent from current state on every backend and must not alone fail a JSONL fork — behavior is backend-equivalent. Introducing a new built-in namespace without declaring its fork semantics must break fork tests, not silently copy or drop state.
- Neither `letty` nor `letty.*` → application: copy on tree, exclude on branch. The only built-in list namespace (`letty.pending.assistant_frame`) excludes; application lists follow the application rule.
...
- Namespace fork knowledge is repeated across `fork.ts`, `values.md` prose, and conformance assertions; nothing forces a new `letty.*` namespace to declare fork semantics.
...
2. The closed classifier/driver of §1.2, exported from core and consumed by all three backends; unknown reserved namespaces (exact `letty` or undeclared `letty.*`) fail the fork only when current surviving state exists, identically on every backend.
...
- current surviving state in an unknown reserved namespace (exact `letty` or an undeclared `letty.*` scalar or surviving list element) fails the fork on every backend; the same namespace **set then deleted** before the fork fails no backend — including JSONL, where the dead history remains as physical lines and the disk fold must classify it as absent. Construct both cases via raw committed writes.
...
- copying usage rows, `letty.result`, or any open-operation state under any option;
...
- one closed core classifier owns every namespace disposition; unknown reserved namespaces (exact `letty` or undeclared `letty.*`) fail forks exactly when current surviving state exists, equivalently on all backends;

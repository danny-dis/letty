...
export class HookRegistry implements Hooks {
	...
	private invokeToolRegistration(
		...
		return startHarnessSpan(
			"letty.harness.hook",
			{
				"letty.lane.name": event.lane,
				"letty.operation.id": event.runId,
				"letty.hook.name": name,
				...(registration.id === undefined ? {} : { "letty.hook.registration_id": registration.id }),
			},
			async (span, spanContext) => {
				try {
					...
					span.setAttributes({ "letty.hook.outcome": blocked ? "blocked" : "completed" });
					return result;
				} catch (error) {
					span.setAttributes({ "letty.hook.outcome": "failed" });

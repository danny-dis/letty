import type { AssistantMessageFrame } from "@letty/letty-ai";
...
export const branchTip = (branch: string) => value<string | null>("letty.branch.tip", branch);
export const branchTipInventoryPrefix = () => value<string | null>("letty.branch.tip");
export const laneConfig = (lane: string) => value<LaneConfiguration>("letty.lane.config", lane);
export const laneState = (lane: string) => value<LaneState>("letty.lane.state", lane);
export const operationResult = (operationId: string) => value<OperationResultRecord>("letty.result", operationId);

export const operationMeta = (operationId: string) => value<OperationMeta>("letty.op.meta", operationId);
export const operationState = (operationId: string) => value<OperationState>("letty.op.state", operationId);
export const operationToolArgs = (operationId: string, stepId: string, sourceIndex: number) =>
	value<Record<string, JsonValue>>("letty.op.tool_args", `${operationId}:${stepId}:${sourceIndex}`);
...
export const operationPreparation = (operationId: string, taskId: string) =>
	value<DurableStructuralPreparation>("letty.op.preparation", `${operationId}:${taskId}`);
...
export const pendingEntry = (entryId: string) => value<PendingEntry>("letty.pending.entry", entryId);
export const pendingToolOutput = (operationId: string, invocationId: string) =>
	value<AgentToolResult<unknown>>("letty.pending.tool_output", `${operationId}:${invocationId}`);
export const pendingAssistantFrames = (operationId: string, responseEntryId: string) =>
	list<AssistantMessageFrame>("letty.pending.assistant_frame", `${operationId}:${responseEntryId}`);

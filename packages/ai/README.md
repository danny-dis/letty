...
```bash
npm install @letty/letty-ai
```

TypeBox exports are re-exported from `@letty/letty-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@letty/letty-ai';
import { builtinModels } from '@letty/letty-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@letty/letty-ai/providers/anthropic';
import { openaiProvider } from '@letty/letty-ai/providers/openai';
import { openrouterProvider } from '@letty/letty-ai/providers/openrouter';
import { amazonBedrockProvider } from '@letty/letty-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@letty/letty-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@letty/letty-ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. letty-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@letty/letty-ai';
...
API-key credentials use the same discriminator as letty's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@letty/letty-ai';
...
```typescript
import { validateToolCall, type Tool } from '@letty/letty-ai';
...
```typescript
import { builtinImagesModels } from '@letty/letty-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@letty/letty-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@letty/letty-ai';
import { openAICompletionsApi } from '@letty/letty-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@letty/letty-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@letty/letty-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are letty thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@letty/letty-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@letty/letty-ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for letty-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for letty-controlled thinking values
...
```typescript
import { createModels, type Context } from '@letty/letty-ai';
import { anthropicProvider } from '@letty/letty-ai/providers/anthropic';
import { openaiProvider } from '@letty/letty-ai/providers/openai';
import { googleProvider } from '@letty/letty-ai/providers/google';
...
```typescript
import { createModels } from '@letty/letty-ai';
import { anthropicProvider } from '@letty/letty-ai/providers/anthropic';
...
- `@letty/letty-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@letty/letty-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@letty/letty-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@letty/letty-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@letty/letty-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@letty/letty-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@letty/letty-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `LETTY_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @letty/letty-ai login              # interactive provider selection
npx @letty/letty-ai login anthropic    # login to specific provider
npx @letty/letty-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@letty/letty-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@letty/letty-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@letty/letty-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@letty/letty-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@letty/letty-ai`.

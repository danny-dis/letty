/**
 * letty-messages API implementation.
 *
 * Streams letty's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "letty-messages"`.
 */
...
export interface LettyMessagesOptions extends StreamOptions {
...
type LettyMessagesUsage = AssistantMessage["usage"];
type LettyMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type LettyMessagesRewriteImpact = {
	policyId: string;
...
/** Serialized assistant-message event as sent by a letty-messages backend. */
export type LettyMessagesEvent =
	| { type: "start" }
	...
	| {
			type: "done";
			reason: Extract<LettyMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: LettyMessagesUsage;
			responseId?: string;
			providerThinkingLevel?: string;
			rewrite?: LettyMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<LettyMessagesStopReason, "aborted" | "error">;
			usage: LettyMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			providerThinkingLevel?: string;
...
export class LettyMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "LettyMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parseLettyMessagesErrorBody(body: string): LettyMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as LettyMessagesErrorBody | null;
...
function formatLettyMessagesResponseError(
	response: Response,
	body: string,
	errorBody: LettyMessagesErrorBody | undefined,
): string {
...
function createLettyMessagesResponseError(
	model: Model<"letty-messages">,
	url: URL,
	response: Response,
	body: string,
): LettyMessagesResponseError {
	const errorBody = parseLettyMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new LettyMessagesResponseError(formatLettyMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): LettyMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: LettyMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
...
function createEventConverter(model: Model<"letty-messages">) {
	...
	return (event: LettyMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readLettyMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<LettyMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseLettyMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseLettyMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseLettyMessagesEvent(raw: string): LettyMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as LettyMessagesEvent) : undefined;
}

function createErrorEvent(model: Model<"letty-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof LettyMessagesResponseError) {
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	return getProviderEnvValue("LETTY_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"letty-messages", LettyMessagesOptions> = (
	model: Model<"letty-messages">,
	context: Context,
	options?: LettyMessagesOptions,
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createLettyMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const lettyEvent of readLettyMessagesEvents(response.body)) {
				const event = convertEvent(lettyEvent);
...
export const streamSimple: StreamFunction<"letty-messages", SimpleStreamOptions> = (
	model: Model<"letty-messages">,
	context: Context,
	options?: SimpleStreamOptions,
): AssistantMessageEventStream => {
	const extra = options as LettyMessagesOptions | undefined;

...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// A profile explicitly configured through letty's auth flow (the `profile`
...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "letty-ai-custom-headers", priority: "low" });
}
...
function addResponseHeadersMiddleware(
	...
	client.middlewareStack.add(middleware, { step: "deserialize", name: "letty-ai-response-headers" });
}
 ...
 * Defaults to "short" and uses LETTY_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("LETTY_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}

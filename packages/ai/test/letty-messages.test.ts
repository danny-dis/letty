...
import { type LettyMessagesOptions, stream, streamSimple } from "../src/api/letty-messages.ts";
...
function createModel(baseUrl: string): Model<"letty-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "letty-messages",
...
describe("letty-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		const { baseUrl, requests } = await startServer({
			headers: { "x-letty-gateway-upstream-provider": "anthropic" },
...
describe("letty-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("letty-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "letty-messages";
		expect(api).toBe("letty-messages");
	});
});

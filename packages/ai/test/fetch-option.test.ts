...
import { streamSimple as streamLettyMessages } from "../src/api/letty-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and letty-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamLettyMessages(createModel("letty-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();

...
const originalAgentDir = process.env.LETTY_CODING_AGENT_DIR;
...
afterEach(() => {
	...
	if (originalAgentDir === undefined) {
		...
		process.env.LETTY_CODING_AGENT_DIR = originalAgentDir;
	}
	closeOpenAICodexWebSocketSessions();
	resetOpenAICodexWebSocketDebugStats();
	vi.useRealTimers();
...
describe("openai-codex streaming", () => {
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "letty-codex-stream-"));
		process.env.LETTY_CODING_AGENT_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("letty");
				expect(headers?.get("User-Agent")).toBe(`letty (${platform()} ${release()}; ${arch()})`);
	...
	// Regression test for https://github.com/letty/letty/issues/9047

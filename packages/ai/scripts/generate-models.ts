...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (model.provider === "openrouter" && model.id.startsWith("inception/mercury-2")) {
		...
		// Letty's low/medium/high pass through verbatim; OpenRouter normalizes to Mercury's vocabulary.
		mergeThinkingLevelMap(model, { off: null });
	}
	...
	try {
		...
		if (data["google-vertex"]?.models) {
			for (const [modelId, model] of Object.entries(data["google-vertex"].models)) {
				...
				// do not match the official Gemini API standard pricing table. letty only accounts
		...
		// models.dev lists the China Coding Plan under this provider ID.
		const kimiModels = data["kimi-code-plan-cn"]?.models;
		if (kimiModels) {
		...
		// models.dev keys are "alibaba-token-plan[-cn]"; letty exposes them as

import type { JsonValue, ServiceCall } from "@letty/chord";
import type { Context, Session, SessionMetadata } from "@letty/letty-agent-core";
import { BACKGROUND_CONTEXT, MemorySessionRepo } from "@letty/letty-agent-core";
			...
			return {
				async invokeService(call, _publish, context) {
					if (
						...
						call.serviceId === "letty.session-management" &&
						call.member === "attach" &&
						call.args.length === 1 &&

import type { ServiceCall } from "@letty/chord";
import { BACKGROUND_CONTEXT, type SessionMetadata } from "@letty/letty-agent-core";
...
describe("Session protocol", () => {
	...
	test("routes opaque server services and publishes attachment changes out of band", async () => {
				...
				attachClient(presentation) {
					return {
						async invokeService(call, _publish, context) {
							if (call.serviceId !== "letty.session-management") throw new Error("Unexpected service");
		...
		await expect(
			client.requestService({ serverId }, { serviceId: "letty.session-management", member: "detach", args: [] }),

import { encodeCbor, encodeClientMessage, encodeFrame, PROTOCOL_VERSION } from "@letty/letty-protocol";
...
test("requires hello as the first message", async () => {
	...
	await client.sendMessage({
		...
		call: { serviceId: "letty.session-directory", member: "list", args: [] },
	});
...
test("accepts fragmented hello and request frames", async () => {
	...
	const request = {
		...
		id: "request-1",
		target: { serverId: "00000000-0000-4000-8000-000000000001" },
		call: { serviceId: "letty.session-directory" as const, member: "list" as const, args: [] as [] },
	};

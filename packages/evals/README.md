...
Letty evals are behavioral, model-backed checks for Letty workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Letty session artifacts.
...
```bash
LETTY_PROVIDER=openai LETTY_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from Letty's normal `ModelRuntime`, including Letty subscription credentials and provider API-key
...
native Letty session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Letty-specific evals use `createLettyCodingAgentHarness(...)` from `src/letty-harness.ts`, with one harness bound
...
const harness = createLettyCodingAgentHarness({ noTools: "all" });

describeEval("Letty smoke", { harness }, (it) => {
...
### Configuring the Letty harness

`createLettyCodingAgentHarness(...)` accepts:
...
- `noTools`: Letty's tool-disable configuration.
...
```ts
const harness = createLettyCodingAgentHarness({
...
prompt creates or changes Letty resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a Letty extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Letty adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Letty configuration:
...
The Letty harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook

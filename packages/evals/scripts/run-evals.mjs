...
const artifactDirectory = process.env.LETTY_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.LETTY_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.LETTY_PROVIDER?.trim() || undefined;
	model = process.env.LETTY_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both LETTY_PROVIDER and LETTY_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	LETTY_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.LETTY_PROVIDER = provider;
	childEnvironment.LETTY_MODEL = model;
} else {
	delete childEnvironment.LETTY_PROVIDER;
	delete childEnvironment.LETTY_MODEL;
}

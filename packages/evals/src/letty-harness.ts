...
import { contentText } from "@letty/letty-ai";
import {
	type AgentSession,
...
import { LETTY_SESSION_SNAPSHOT_ARTIFACT } from "./vitest-evals/artifacts.ts";

export type LettyCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type LettyCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type LettyCodingAgentHarnessOptions = {
	name?: string;
	model?: LettyCodingAgentModelSelection;
...
type LettyCodingAgentHarnessWithOutput<TOutput extends JsonValue> = LettyCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: LettyCodingAgentModelSelection | undefined,
	environment: { LETTY_PROVIDER?: string; LETTY_MODEL?: string } = process.env,
): LettyCodingAgentModelSelection {
	const provider = (explicitModel?.provider ?? environment.LETTY_PROVIDER)?.trim();
	const id = (explicitModel?.id ?? environment.LETTY_MODEL)?.trim();
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both LETTY_PROVIDER and LETTY_MODEL as defaults.");
	}
	return { provider, id };
}
...
async function runLettyCodingAgent<TOutput extends JsonValue>(
	input: LettyCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: LettyCodingAgentHarnessOptions | LettyCodingAgentHarnessWithOutput<TOutput>,
	...
	const root = await mkdtemp(join(tmpdir(), "letty-eval-"));
	...
	try {
		...
		try {
			...
			if (response === undefined) throw new Error("Letty eval input must include at least one prompt step.");
	...
	if (sessionManager) {
		try {
			...
			if (sessionPath && existsSync(sessionPath)) {
				setArtifact(LETTY_SESSION_SNAPSHOT_ARTIFACT, await readFile(sessionPath, "utf8"));
			}
		} catch (error) {
...
export function createLettyCodingAgentHarness<TOutput extends JsonValue>(
	options: LettyCodingAgentHarnessWithOutput<TOutput>,
): Harness<LettyCodingAgentInput, TOutput>;
export function createLettyCodingAgentHarness(
	options?: LettyCodingAgentHarnessOptions,
): Harness<LettyCodingAgentInput, string>;
export function createLettyCodingAgentHarness<TOutput extends JsonValue>(
	options: LettyCodingAgentHarnessOptions | LettyCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<LettyCodingAgentInput, string | TOutput>({
		name: options.name ?? "letty-coding-agent",
		run: ({ input, signal, setArtifact }) => runLettyCodingAgent(input, signal, setArtifact, options),
	});
}

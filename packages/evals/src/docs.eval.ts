...
import { Type } from "@letty/letty-ai";
import { defineTool } from "@letty/letty-coding-agent";
...
const documentationPages = globSync("**/*.md", { cwd: docsRoot })
	...
	.sort()
	.map((path) => ({ path }));
const documentationAuditHarness = createLettyCodingAgentHarness({
...
describeEval("Coding agent documentation", { harness: documentationAuditHarness }, (it) => {
	it.for(documentationPages)("$path matches the implementation", { timeout: 300_000 }, async ({ path }, { run }) => {
		...
		const result = await run(`Audit one Letty documentation page against the repository implementation.

Documentation page: ${documentationPath}
Repository root: ${repositoryRoot}

Read the complete documentation page. Identify its concrete claims about Letty behavior, public APIs, configuration, commands, formats, defaults, and supported values.

...
import { LETTY_SESSION_SNAPSHOT_ARTIFACT, persistEvalArtifactReferences } from "./artifacts.ts";
...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.LETTY_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(
			([name]) => name !== "runId" && name !== LETTY_SESSION_SNAPSHOT_ARTIFACT,
		),
	);
	const record = {
		schemaVersion: 1,

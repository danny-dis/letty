...
export const LETTY_SESSION_SNAPSHOT_ARTIFACT = "lettySessionJsonl";

const evalSessionArtifactKey = Symbol("letty-evals-session-artifact");
const evalSourceArtifactKey = Symbol("letty-evals-source-artifact");

interface LettySessionAttachment extends TestAttachment {
	name: "session.jsonl";
...
interface LettySessionArtifact extends TestArtifactBase {
	type: "@letty/letty-evals:session";
	runId: string;
	attachments: [LettySessionAttachment] | [];
}

interface SourceArtifact extends TestArtifactBase {
	type: "@letty/letty-evals:source";
	runId: string;
...
declare module "vitest" {
	interface TestArtifactRegistry {
		[evalSessionArtifactKey]: LettySessionArtifact;
...
export async function recordEvalSessionArtifact(
	...
	const session = run.artifacts?.[LETTY_SESSION_SNAPSHOT_ARTIFACT];
	if (session === undefined) return;
	if (typeof runId !== "string" || typeof session !== "string") {
		throw new TypeError("Letty eval session artifact metadata is invalid.");
	}
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@letty/letty-evals:session" && artifact.type !== "@letty/letty-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@letty/letty-evals:session" ? "sessions" : "sources";

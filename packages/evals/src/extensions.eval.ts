...
import { createLettyCodingAgentHarness, type LettyCodingAgentInput } from "./letty-harness.ts";
...
type ExtensionAuthoringOutput = {
	...
	systemPromptHasLettyDocs: boolean;
	...
	return createLettyCodingAgentHarness({
		...
		output: ({ response, session }) => {
			...
			return {
				...
				systemPromptHasLettyDocs: session.systemPrompt.includes("\nLetty documentation (read only"),
...
function excludeGuidelinesAndDocumentation(defaultPrompt: string): string {
	...
	if (guidelinesStart === -1) throw new Error("Default Letty system prompt has no Guidelines section.");
...
function prepareDefaultPromptOverride(defaultPrompt: string): string {
	...
	if (cwdStart === -1) throw new Error("Default Letty system prompt has no working-directory section.");
	return defaultPrompt.slice(0, cwdStart);
}

const ExtensionAuthoringJudge = createJudge<LettyCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@letty/letty-coding-agent")) {
				failures.push("extension does not import the canonical @letty/letty-coding-agent package");
			}
...
const extensionHarnessTable = evalHarnessTable("Letty extension authoring system prompt", {
...
describe.for(extensionHarnessTable)("$name", ({ harness }) => {
	describeEval(
		"Letty extension authoring system prompt",
		...
		(it) => {
			it("creates, reloads, and uses a hello extension", async ({ run, task }) => {
				const result = await run([
					{
						type: "prompt",
						content:
							"Create a Letty extension with a hello tool that takes a name and returns a greeting. For example, passing Bob should return `Hello, Bob!`.",
					},
					{ type: "reload" },
					{
						type: "prompt",
				...
				if (result.output.extensionSource !== null) {
					...
					if (typeof runId !== "string") throw new Error("Letty eval run did not record a run ID.");
				...
				expect(result.output.systemPromptHasLettyDocs).toBe(expectsFullPrompt);
			});
		},
	);
});

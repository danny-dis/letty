# @letty/letty-telemetry

Vendor-neutral telemetry contracts and typed schema utilities for letty packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Letty packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Letty Package Integration](#letty-package-integration)
...
```bash
npm install @letty/letty-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@letty/letty-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@letty/letty-telemetry';
...
`@letty/letty-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@letty/letty-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@letty/letty-agent-core';
...
## Letty Package Integration

Package ownership is intentionally split:

- `@letty/letty-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@letty/letty-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@letty/letty-agent-core` owns and exports the letty AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
import {
  ...
  startAiSpan,
  startHarnessSpan,
} from '@letty/letty-agent-core';
```

The letty schemas use letty-owned `letty.ai.*`, `letty.harness.*`, and `letty.session.*` names. Adapters may translate them to backend conventions without changing the emitted letty vocabulary.
